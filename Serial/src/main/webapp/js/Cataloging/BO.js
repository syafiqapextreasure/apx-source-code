...
function addTag(tableID,ctmstr) {
	...
	if(selected.length==0 && $("#subfields option").length>0){
		swal('', 'Please select atleast one subfield', '');
	}else{
	//Create random key 
	...
	var manda = $(".tags :selected").attr('data-manda');
	...
	/*$('#BibRcrd > tbody').each(function() {
	    ...
	    if (Number(tag) > Number(tags)) {
	     ...
	     row1 = newTbody.insertRow(rowCount1);
	   /* }
	...
	if(indi1=="NoIndicator" || indi2=="NoIndicator"){
		  ...
		  var cell3 = row1.insertCell(2);
		   cell3.innerHTML ="<input size='1' maxlength='1' style='text-align:center;display:none' type='text' class='indiValue2' data-tag='"+tag+"' data-indilvl='2' onkeyup='updateIndi(this)' id='indi2_"+indi2+new_key+"' value=&#32; disabled>";
			...
			var cell4 = row1.insertCell(3);
	    	 ...
	    	 cell7.setAttribute("data-manda",manda);
		 $('#subfields option:selected').each(function() {
		     ...
		     cell8.innerHTML  = "<a data-toggle='modal' data-indiid1='"+indi1+new_key+"' data-indiid2='"+indi2+new_key+"' data-ctmstr='"+ctmstr+"' data-id='"+tags+new_key+"' data-target='#viewSubfields' data-indi1='"+indi1+"'"+
		     					"data-indi2='"+indi2+"' data-tagdesc='"+tagdesc+"' data-tag='"+tags+"' data-subf='"+ cell7.innerHTML+"' data-manda='"+ manda+"' onclick='openModal(this)'" + 
		 ...
		 if(tags=="000"||tags=="006"||tags=="007"||tags=="008"){
			 ...
			 if(tags==("001")||tags==("002")||tags==("003")||tags==("004")||
						tags==("005")||tags==("009")||tags==("010")){
			 cell8.innerHTML  = "<a data-toggle='modal' data-indiid1='"+indi1+new_key+"' data-indiid2='"+indi2+new_key+"' data-id='"+tags+new_key+"' data-ctmstr='"+ctmstr+"' data-target='#viewSubfields' data-indi1='"+indi1+"' data-indi2='"+indi2+"' data-tagdesc='"+tagdesc+"' data-tag='"+tags+"' onClick='openCtrlField(this)' class='btn btn-info btn-sm subfButton' ><span class='glyphicon glyphicon-pencil' title='View Subfields'></span></a>";
		 }
		 			var $sort = this;
				    var $table = $('#BibRcrd');
				    ...
				    $rows.sort(function(a, b){
				        ...
				        if($($sort).hasClass('asc')){
				            return (parseInt(keyA.trim()) > parseInt(keyB.trim())) ? 1 : 0;
				        } else {
				            return (keyA.trim() > keyB.trim()) ? 1 : 0;
				        }
				    });
		  ...
		  //var new_key = CreateKey();
		   ...
		   var name = key;
		    ...
		    row.id = $(this).val() + newID;
		     ...
		     cell3.colSpan = 2;
		     cell3.innerHTML ="<input size='99' type='text' onfocus='getDesc(this);getData(this)' name='"+name+"' data-desc='|"+$(this).text()+"' id='|"+$(this).val()+"' style='width:100%'>";
		 ...
		 $.get("Duplicate_Record",{data:$(this).val(), tag: tag, action:"keyInData"},function(data_title){
			 if(data_title.trim() =="true"){
				 localStorage.setItem("duplicate", "Y");
				  alertMessage("Successful", "success", "111",tag, "@tag"); 
			 }else{

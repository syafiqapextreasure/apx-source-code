...
function accept(){
	...
	var total = 0.00;
      ...
      for(var i=0;i<rows.length;i++)
      {
    	  ...
    	  vendor = $(rows[i]).find("td").eq(0).text();
    	   total = parseFloat(total) + parseFloat($(rows[i]).find("td").eq(3).text());
    	   //total.push("'" + order + "'");
      }
      
      var ref = chequeno + ":" + chequdate;
          
      $.get("FundDistribution",{module:"AAAM1150", vendor:vendor, ref:ref,totalOrder:total, desc:"Serial : Record Payment"},function(data){
...
function processRecord(){
	...
	var currency = $('#orderingList tr:eq(1) > td:eq(5)').text();
 ...
 /*     $.get("FundDistribution",{module:"AAAM1150", vendor:vendor, ref:chequeno,totalOrder:total, desc:"Serial : Record Payment"},function(data){
    	  alert("sss");
  		if(data.trim()!="no_funddistribution"){
  		 $("#fundDistribution").modal("show");
  		 $("#fundContent").html(data);
...
/*	 
	 ...
	 $("#orderingList").DataTable().destroy();
		$.get("Table_RecordPayment",{textValue:textValue, criteria:checked},function(data){
			$("#voucherno, #chequeno").removeAttr("disabled"); 
            	...
            	$( "#chequedate" ).datepicker( "option", "dateFormat", "dd/mm/yyyy" );
         	   $('#chequedate').datepicker('setDate', new Date());
         	  $( "#voucherdate" ).datepicker( "option", "dateFormat", "dd/mm/yyyy" );
        	   $('#voucherdate').datepicker('setDate', new Date());
        		if($("#chequedate").val()!=""){
        			$(".chequedate").text($("#chequedate").val());
        		}
        		if($("#voucherdate").val()!=""){
        			$(".voucherdate").text($("#voucherdate").val());
        		}

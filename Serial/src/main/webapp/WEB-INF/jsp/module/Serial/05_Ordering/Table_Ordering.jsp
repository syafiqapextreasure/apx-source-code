...
<%@page import="com.ilmu.serial.Ordering.*"%>
...
for(Ordering i : ordering){
	System.out.println("Order" + i.getSE03ORDER());
...
<td>
		...
		%>
		<input onchange="getValue()" name= "orderlist" type = "checkbox" class="orderlist" id = "orderlist" value="<%=i.getSE03ORDER()%>*" data-total="<%=i.getTotal() %>">
		<%
		}else{
		%>
		<input onchange="getValue()" name= "orderlist" type = "checkbox" class="orderlist" id = "orderlist" value="<%=i.getSE03ORDER()%>" data-total="<%=i.getTotal() %>">

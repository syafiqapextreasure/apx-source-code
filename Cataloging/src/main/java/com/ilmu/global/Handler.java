...
public class Handler {
    ...
    public static String isNull(String nullValue) {
        StringBuilder result = new StringBuilder();
        System.out.println("dd23" + nullValue);
        if (nullValue == null || nullValue.equals("NULL")) {
            System.out.println("dd" + nullValue);
            result.append("");
        } else {
            System.out.println("dd11" + nullValue);
            result.append(nullValue);
        }
        return result.toString();
    }
    ...
    public static String convertIndi(String nullValue) {
        ...
        if (nullValue == null) {
            System.out.println("dd" + nullValue);
            result.append("#");
        } else if (nullValue.trim().isEmpty()) {
            System.out.println("dd12" + nullValue);
            result.append("#");
    ...
    public static double parseDecimal(String original) {
        System.out.println("SS" + original);
        double cost = 0.0;
    ...
    public static String removeNumberSubf(String raw) {
        String[] rawArray = raw.split("\\|");
        StringBuilder result = new StringBuilder();
        if (raw != null) {
            int i = 1;
            while (i < rawArray.length) {
                ...
                if (rawArray[i].isEmpty()) {
                    System.out.println("Data12");
                    result.append("");
                } else if (!rawArray[i].substring(0, 1).matches("6") && !rawArray[i].substring(0, 1).matches("8") && (splitData = rawArray[i].substring(1)) != null && splitData != "") {
                    result.append(splitData);
                    if (i != rawArray.length) {
                        result.append(" ");
                    }
    ...
    public static String removeSubf(String raw) {
        String[] rawArray = raw.split("\\|");
        StringBuilder result = new StringBuilder();
        if (raw != null || raw != "") {
            int i = 1;
            while (i < rawArray.length) {
                ...
                if (rawArray[i].isEmpty()) {
                    result.append(" ");
                } else if (!rawArray[i].substring(0, 1).equals("6") && !rawArray[i].substring(0, 1).equals("8") && (splitData = rawArray[i].substring(1)) != null && splitData != "") {
        ...
        System.out.println("Subfield1" + result.toString());
        return result.toString();
    }

    public static String splitUser(String username) {
        String[] arrSplit = username.split("-");
        int i = 0;
    ...
    public static String removeSubfield(String raw) {
        ...
        if (raw != null) {
            ...
            while (i < rawArray.length) {
                ...
                String splitData = rawArray[i].substring(1);
                System.out.println("spliData: " + splitData);
                if (splitData != null && splitData != "" && i != rawArray.length) {
                    if (i > 1) {
                        result.append(" ");
                    }
                    result.append(splitData);
    ...
    public static String changeData(String raw) {
        String[] arrOfStr;
        StringBuilder result = new StringBuilder();
        String[] stringArray = arrOfStr = raw.split("[^A-Za-z0-9\\s+$%]");
        int n = arrOfStr.length;
        int n2 = 0;
        while (n2 < n) {
            String a = stringArray[n2];
            System.out.println("Sentence" + a.trim() + "sd");
    ...
    public static String removeSpecialChar(String data) {
        if (!data.isEmpty()) {
            data = data.replaceAll("[^A-Za-z0-9% ][^\\p{N}\\p{P}\\p{Z}\\p{L}\\p{M}*]+", "");
    ...
    public static String convertTabNewLine(String data) {
        if (data.contains("\\t")) {
            data = "    ";
    ...
    public static final String toRoman(int number) {
        System.out.println("geeqwqw" + number);
        int l = map.floorKey(number);
        if (number == l) {
            return map.get(number);
        }
        System.out.println("gee" + number);
        System.out.println("gee" + map.get(l) + Handler.toRoman(number - l) + ")");
        return String.valueOf(map.get(l)) + Handler.toRoman(number - l);
    }

    public static String marcType(String module) {
        String marc = "";
        if (module.equals("Authority")) {
            marc = "Y";
        } else if (module.contains("Cataloging")) {
            marc = "U";
        }
        return marc;
    }

    public static String bibType(String module) {
        String marc = "";
        if (module.equals("Authority")) {
            marc = "Y";
        } else if (module.contains("Cataloging")) {
    ...
    public static String tblName(String module) {
        String tblname = "";
        if (module.equals("Authority")) {
            tblname = "AU";
        } else if (module.contains("Cataloging")) {

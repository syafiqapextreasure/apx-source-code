...
public class ReleaseForCirculation {
    ...
    public static String getRecordStat(String CT03DOCNO) {
        String query = "Select CT03STAT FROM CTDOCM WHERE CT03DOCNO='" + CT03DOCNO + "'";
        System.out.println(query);
        String status = "";
        try {
            try {
    ...
    public static String getReservedPatron(String matno) {
        String query = "Select ci03patr, gl14name from ciresv, glpatr where gl14patr=ci03patr and ci03matno = '" + matno + "'";
        System.out.println(query);
        String patron = "";
        try {
            try {
                ...
                rs = stmt.executeQuery(query);
                while (rs.next()) {
                    patron = String.valueOf(rs.getString("ci03patr")) + "," + rs.getString("gl14name");
                }
            }
            catch (SQLException e) {
                e.printStackTrace();
    ...
    public static List<ReleaseForCirculation> Title(String CT03DOCNO) throws SQLException {
        ...
        try {
            try {
                ...
                rs = stmt.executeQuery(query);
                while (rs.next()) {
                    ReleaseForCirculation new01_TemplateMaintenance = new ReleaseForCirculation(ISBD.getSubfield(rs.getString("CT05SRAW"), isbd), Integer.parseInt(rs.getString("CT05POINTER")), rs.getString("CT03MATNO"));
    ...
    public static boolean updateResv(String CT03DOCNO, String matno) throws SQLException {
        boolean isSuccess;
        block9: {
            ...
            if (db.getDBType().equals("mssql")) {
                query = "Select TOP 1 CI03PATR from CIRESV where CI03MATNO='" + matno + "'";
            } else if (db.getDBType().equals("oracle")) {
                query = "Select CI03PATR from CIRESV where CI03MATNO='" + matno + "' AND ROWNUM=1";
            }
            ...
            try {
                try {
                    ...
                    while (rs.next()) {
                        HashMap<String, String> valueStr = new HashMap<String, String>();
                        HashMap<String, String> condition = new HashMap<String, String>();
                        condition.put("CI03PATR", rs.getString("CI03PATR"));
                        condition.put("CI03MATNO", matno);
                        valueStr.put("CI03NDATE", DateTime.displayToDBFormat(DateTime.getTodayDate()));
                        valueStr.put("CI03DDATE", newDate);
                        valueStr.put("CI03DOCNO", CT03DOCNO);
                        valueStr.put("CI03NSTAT", "A");
                        String querys = QueryBuilder.createUpdateQuery("CIRESV", valueStr, null, null, condition);
                        System.out.println("Query" + query);
                        isSuccess = DBQuery.runQuery(querys);
                    }
                }
                catch (SQLException e) {
                    e.printStackTrace();
    ...
    public static String getRecordStatdesc(String status) {
        String query = "SELECT GL36DESC FROM GLDOCS WHERE GL36STAT = '" + status + "'";
        System.out.println(query);
        String statusdesc = "";
        try {
            try {
                ...
                rs = stmt.executeQuery(query);
                while (rs.next()) {
                    statusdesc = rs.getString("GL36DESC");
                }
            }

 ...
 */
$("[title]").tooltip();
function addTplNTplInfo(tplID, tplName, status, type, marc){
						...
						function(message){
							if(message.trim()=="Successful"){
								...
								alertMessage("Successful", "success", "005","added template", "@action");
								  ...
								  $('body').css('padding-right', '');
								  $("#addRcrd").hide();
									var url = "ShowTplDetails?seqNo="+tplID + "&tplName="+encodeURI(tplName)+ "&module=" + module;
...
//Collapse panel to show values for each template
		function myFunction(newID){
			...
			var url = "ShowTplDetails?seqNo="+details[0] + "&tplName=" + details[1]+ "&module=" + details[2].trim() ;
				...
				$.get(url,function(data_title)
						{
							...
							$("#display_title").html(data_title);
							 ...
							 $('#btn_addTplInfo').attr('href',"AddNewTplInfo?action=2&tplName="+encodeURI(details[1].trim())+
										"&seqNo="+details[0] + "&module=" + details[2].trim() + "&GL17TAG=" + tags.toString());
	 ...
	 { 
		 ...
		 $('#addTplInfo').click(function(){
			  ...
			  //Push selected subfield options
	           ...
	           $.get("Handler_BOValidation",{validation:"MandaTag", marc:marc},function(tag){
	        	 if(tag.trim().length!=0){
	        	   ...
	        	   if(exist==true&&name!=null){
	        		 ...
	        		 if(action=="upAction"){
						...
						addTplNTplInfo(tplID, tplName, status, type, marc);
						$("#addRcrd").modal("hide");
	        		 }
	        	   }else if(exist==true){
	        		   var tplName = $('#tplname').val();
			...
			$.ajax({
	   			...
	   			success: function(result) {
  					...
  					$('#updateModal1').modal('hide');
	  					...
	  					var url = "ShowTplDetails?seqNo="+$('#seqNo').val() + "&tplName="+ $('#tplName').val() + "&module=" + module;
		...
		/*To retrieve Indicators and Subfields*/
		 function getIndicators(){
			  ...
			  var module = $("#module").val();
				...
				var url = "Indicators_Handler?GL18TAG=" + tag + "&module=" + module;
				$.get(url,function(data){
			    				      ...
			    				      $.ajax({
			    				 			...
			    				 			success: function(result) {
			    				 				...
			    				 				if(status == "ok"){
			    				 					...
			    				 					// location.reload();
			    				 					var url = "ShowTplDetails?seqNo="+tplName + "&tplName="+encodeURI(tpl)+ "&module=" + module;

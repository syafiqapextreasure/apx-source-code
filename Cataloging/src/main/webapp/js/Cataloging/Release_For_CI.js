$(document).ready(function(){  
	...
	$('#accMatNo').unbind().keydown(function(event){
    			...
    			$.get("ReleaseCirculation",{CT03DOCNO:docNo},function(status){
    				...
    				if(splitStatus[0]=="error"){
               		...
               		var label = "@docNo";
    		  		  ...
    		  		  }else if(splitStatus[0]=="error1"){
      		  			var code = "136";
                   		var criteria = docNo;
                   		var label = "@docNo";
        		  			  $.get("Error_Message?GL79ERRCODE="+code+"" + 
        	               				"&criteria=" + criteria + "&label="+label+"",function(msg){
        		  				  		msg = msg.replace("@patron",  splitStatus[4]);
        				        	   $('#releaseList').append("<tr><td>" + docNo + "</td>" + 
        					   			            "<td>" + msg + "</td>" + 
        					   			         "<td>" + splitStatus[1] + "</td>" + 
        					   			      "<td>" + splitStatus[2] + "</td>" + 
...
/*	   $('#accMatNo').on('keypress', function(event) {
	 ...
	 //Display list of accession no in modal
	   ...
	   $('#releaseBtn').unbind().click(function() {
			...
			if(docNo.length > 0){
			var print = $('#print').is(':checked');
			var reserved = $('#reserve').is(':checked');
	
			$.get("ReleaseCirculation",{CT03DOCNO:docNo},function(status){
		//		alert("status"+status);
				var splitStatus = status.trim().split("?");
		//		alert("status2"+splitStatus[4]);
				if(splitStatus[0]=="error"){
		  			var code = "001";
           		var criteria = docNo;
           		var label = "@docNo";
		  			  ...
		  			  $.get("Error_Message?GL79ERRCODE="+code+"" + 
							...
							$.get("Handler_CIRESV",{CT03DOCNO:docNo, CT03MATNO:splitStatus[3],action:"update"}
								...
								if(status.trim()=="ok"){
										...
										if (print){
											printLetter(docNo,splitStatus[1],splitStatus[2],splitStatus[3],print);
										}
									}
									});
								}
	   ...
	   function printLetter(accMatNo, accTitle, accCallNo, matno,print){

...
$('#CT03DOCNO, #editCT03DOCNO').keyup(function(){
 	...
 	function updateAccession(){
 		...
 		var oriCT03DOCNO = $('#oriCT03DOCNO').val();
   		...
   		$.ajax({
   			...
   			success: function(result) {
  				...
  				if(updateRecord == "ok"){
  					...
  					$('#editAccMaintenance').modal('hide');

  					$.get("Table_SearchByAccNo",{txtMatNo : $('#editCT03DOCNO').val()},function(data){
  						
  						$('#' + $('#oriCT03DOCNO').val()).replaceWith(data); 
 ...
 function addRecord(record){
	...
	/*	var maxlength = $("#maxlength").val();
		...
		//if(CT03DOCNO.length>maxlength || CT03DOCNO.length<minlength){
		/*if(CT03DOCNO.length<10){
			swal('', "Invalid accession no." , "");
		}else{*/
		...
		$.ajax({
				...
				success: function(result) {
					...
					if(updateRecord == "ok"){
						...
						var text = $("#searchbyaccno tbody tr:first").text();
						$.get("Table_SearchByAccNo",{txtMatNo : CT03DOCNO},function(data){
				
							if(text=="No data available in table"){
								$(".accessionList").html(data);
							}else{
								$(".accessionList").append(data);
							}
						...
						//reloadRecord();
						 ...
						 alertMessage("", "info", "019",null, null);
					}else if(updateRecord == "acclenwrg"){
						swal('', "Invalid accession no." , "");
					}
				}
			});
	...
	$.get("SearchVendor",{criteria:matno,action:"keyupCtrlNo"},function(title){
		if(title.trim()=="Error"){
			$(".title").html("<font style='color:red'>Invalid control no.</font>");
			 $("#add-submit").prop('disabled', true);
			 $('.marcbtn').attr("disabled","disabled");
		}else{
			$(".title").html(title);
			 $("#add-submit").prop('disabled', false);
			 $('.marcbtn').removeAttr("disabled");
			   $(".marcbtn").attr("href", "Modal_BORcrd?ctrlno=" + matno);
		}
	   });	
}
...
function vendorFocusOut(vendor){
	var vendor_details = $(vendor).val();
...
function vendorKeydown(event,vendor){
...
function equalForex(forex){
	$(".CT03RATE").val($(forex).find(':selected').attr('data-id'));
...
function updateForeignCost(localCost){
	var result = $(".CT03RATE").val()*$(localCost).val();
	$(".localCost").val(result.toFixed(2));
}

function updateLocalCost(localCost){
...
$(document).ready(function() {
	...
	$("#addAccMaint").on("hide.bs.modal", function(){
		 $(this).removeData();
	});
	...
	$("#editAccMaintenance").on('hide.bs.modal', function(){
		 $(this).removeData();
	});
	
	$("#viewAccMaintenance").on('hide.bs.modal', function(){
		 $(this).removeData();
	});

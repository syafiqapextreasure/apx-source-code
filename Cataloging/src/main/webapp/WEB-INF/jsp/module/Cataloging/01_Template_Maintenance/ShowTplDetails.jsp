										...
										<td>
												...
												<tr>
													<td>
														...
														%>
														<input type="hidden" id="module1" value="<%=module%>">
											...
											%>	
											<td>
												<a data-toggle='modal' data-target='#updateModal1' href="Modal_EditTplInfo?updateId=<%=String.valueOf(e.getCTTPLID())%>&seqNo=<%= request.getParameter("seqNo") %>&tag=<%= e.getCTTPLTAG() %>&marc=<%= e.getCT15MARC() %>"  class="btn btn-info btn-sm"><span class="glyphicon glyphicon-pencil" title="Update Template"></span></a>
								            	<button onclick="deleteTplPlugin(this.id)" id="<%=String.valueOf(e.getCTTPLID())%>,<%=e.getCTTPLN()%>,<%=e.getCTTPLTAG() %>,<%= module%> " class="btn btn-dull btn-sm" disabled><span class="glyphicon glyphicon-trash" title="Delete Template Details"></span></button>
											</td>
											<%
											}
								 			else{
											...
											<td>
								                <a data-toggle='modal' data-target='#updateModal1' href="Modal_EditTplInfo?updateId=<%=String.valueOf(e.getCTTPLID())%>&seqNo=<%= request.getParameter("seqNo") %>&tag=<%= e.getCTTPLTAG() %>&marc=<%= e.getCT15MARC()%>"  class="btn btn-info btn-sm"><span class="glyphicon glyphicon-pencil" title="Update Template"></span></a>
								                <a onclick="deleteTplPlugin(this.id)" id="<%=String.valueOf(e.getCTTPLID())%>,<%=Integer.parseInt(request.getParameter("seqNo"))%>,<%=e.getCTTPLTAG() %>,<%=module%>" class="btn btn-dull btn-sm"><span class="glyphicon glyphicon-trash" title="Delete Template Details"></span></a>
											 </td>
											<%
								 			}
											%>

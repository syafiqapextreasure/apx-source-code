...
<%
	...
	System.out.println("SSS11" +szRequestPage);
...
function download(x, y) {
   ...
   // submitForm2(form, 'z3950','zDownloadMarc');
   var page = $("#page").val();
    var filename = $("#filename").val();
    var hdSelectedMARC = 0;
    var hdSelectedServer = 0;
    
    if(x==""&& y==""){
    	hdSelectedMARC = $("#hdSelectedMARC").val();
    	hdSelectedServer = $("#hdSelectedServer").val();
    }else{
    	hdSelectedMARC =x;
    	hdSelectedServer = y;
    }
    ...
    $.get("zDownloadMarc",{page:page,filename:filename, hdSelectedMARC:hdSelectedMARC, hdSelectedServer:hdSelectedServer},function(data_title){

        	/* 	localStorage.clear();
        	localStorage.setItem('data_title', data_title.trim()); */
        	$(".z3950Value").val();
        	$(".z3950Value").val(data_title.trim());
        	$('a[id="z3950"]').attr('disabled','disabled');
        	$("#status").html('<span class="label label-success">Download completed</span>');	
        });
}
...
function submitForm2(form, module, action) {
   ...
   document.getElementById("div_Z390").style.display = "block";
   $("#filename2").text(filename);
   $('input:radio[name="modal_MARC"][value="MARCS"]').attr('checked',true);
   $('a[id="z3950"]').attr('disabled','disabled');
    return false;
}
...
%>
 <%
  		...
  		Date date = new Date();
  		String filename = "MARC_" + dateFormat.format(date) + ".txt"; 
  		System.out.println("SSS" + request.getParameter("hdSelectedMARC") + request.getParameter("hdSelectedServer"));
  	%>
</table>
<form name="frmZView" method="get">
  <input type="hidden" name="page" id="page" value="<%= szAction %>">
  <input type="hidden" id="filename" name="filename" value="<%=filename%>">
  <input type="hidden" id="hdSelectedMARC" name="hdSelectedMARC" value="<%= request.getParameter("hdSelectedMARC") %>">
  <input type="hidden" id="hdSelectedServer" name="hdSelectedServer" value="<%= request.getParameter("hdSelectedServer") %>">
</form>

...
if (next && action.equals("DuplicateChk")) {
	...
	if (duplicate) {
		out.println(duplicate + ",Cataloging");
		next = false;
		break;
	} else {
		next = true;
...
if (existMatno) {
	String security = Bibliography.getPrivate(matno);

	queryList.add(Indexing.updateCTMATM(matno, modiby, DateTime.getTodaySystemDate(),
	DateTime.timeToDBFormat(DateTime.getTodayTime()), type, tag006, tag007, tag008, Handler.tblName(module),
	security));

} else {

	queryList.add(Indexing.addCTMATM(matno, createby, createdate, creatime, modiby, DateTime.getTodaySystemDate(),
	DateTime.timeToDBFormat(DateTime.getTodayTime()), type, tag006, tag007, tag008, Handler.tblName(module),
	"N"));
}
...
for (int i = 0; i < total; i++) {
	...
	if (raw[i] != null) {
		...
		if (tag[i].equals("006") || tag[i].equals("007") || tag[i].equals("008") || tag[i].equals("001")
		|| tag[i].equals("002") || tag[i].equals("003") || tag[i].equals("004") || tag[i].equals("005")
		|| tag[i].equals("009")) {
	...
	if (tag[i].equals("006") || tag[i].equals("007") || tag[i].equals("008")
							|| tag[i].equals("001") || tag[i].equals("002") || tag[i].equals("003")
							|| tag[i].equals("004") || tag[i].equals("005") || tag[i].equals("009")) {
		rawnew = raw[i];
	}else{
		rawnew = Handler.convUpperCase(Handler.removeNumberSubf(raw[i]).replaceAll("\\p{Punct}", ""));
	}

	///boolean existCT05SCONV = Indexing.CT05SCONV_Exist(raw[i], tag[i]);
	List<Indexing2> existCT05SCONV = Indexing2.find5SRAW(tag[i],
			///Handler.convUpperCase(Handler.removeNumberSubf(raw[i]))
			rawnew);
	
	
	System.out.println(tag[i]+"ine 176 handler_idexing"+existCT05SCONV.size()+" : " +rawnew);

	if (!tag[i].trim().equals("000")) {
			//List<Indexing> rawdata = Indexing.checkData(CT04SCONV, tag[i]);


			if (existCT05SCONV.size() != 0) {
				boolean continueloop = false;
				System.out.println("ine 181 lagi besar saiz dari 0");
				for (Indexing2 j : existCT05SCONV) {
					String compareStrg = "";
					if (tag[i].equals("006") || tag[i].equals("007") || tag[i].equals("008")
							|| tag[i].equals("001") || tag[i].equals("002") || tag[i].equals("003")
							|| tag[i].equals("004") || tag[i].equals("005") || tag[i].equals("009")) {
					...
					System.out.println(tag[i] + " : " +j.getCT05SRAW().replaceFirst("\\s++$", "")+"line 199 handler_indexing : " + compareStrg);
					
					System.out.println("ine 200 handler_idexing SCONV == row[i]"
							+ j.getCT05SRAW().replaceFirst("\\s++$", "")
									.equals(compareStrg));
					
					if(!compareStrg.equals("")){
						if (j.getCT05SRAW().replaceFirst("\\s++$", "")
								.equals(compareStrg)) {
							

							queryList.add(Indexing.updateCT05HITS(Integer.parseInt(j.getCT05POINTER()), tag[i]));
							
							queryList.add(Indexing.addCTPONT(Integer.parseInt(j.getCT05POINTER()), matno, tag[i], indicator1, indicator2, status, Handler.marcType(module),
				...
				if (continueloop) {
					...
					System.out.println(Handler.convUpperCase(Handler.removeNumberSubf(raw[i]))+"ine 219 handler_idexing tag apa ye : "+tag[i]); 
					int totalcount = Indexing.TotalSCONVRcrd(CT04SCONV, tag[i]);
					if (tag[i].equals("006") || tag[i].equals("007") || tag[i].equals("008") || tag[i].equals("001")
							|| tag[i].equals("002") || tag[i].equals("003") || tag[i].equals("004")
							|| tag[i].equals("005") || tag[i].equals("009")) {

						//queryList.add(Indexing.addCTTable(pointer.getGL98VALUE(), raw[i], raw[i], tag[i], totalcount, 1, "T"));
						queryList.add(Indexing.addCTTable(pointer.getGL98VALUE(),
								GlobalValidation.removeSubfWithNoRecord(raw[i]),
								Handler.convUpperCase(Handler.removeNumberSubf(raw[i])), tag[i],
								(existCT05SCONV.size() + 1), 1, "T"));
					} else {
					...
					queryList.add(Indexing.addCTPONT(pointer.getGL98VALUE(), matno, tag[i], indicator1, indicator2,
							status, Handler.marcType(module), Handler.tblName(module)));

				}

			} else {
				System.out.println("line 243 handler_indexing : xda lagiiiiiiii");
				String columnName = "CATPOINTERNO";
				Glnumb pointer = Glnumb.getGL98VALUE(columnName);
				...
				if (tablenames.contains(Bibliography.getMarkTagTableName(tag[i]))
						&& duplicateData.contains(CT04SCONV)) {
				...
				if (tag[i].equals("006") || tag[i].equals("007") || tag[i].equals("008") || tag[i].equals("001")
					...
					/*queryList.add(Indexing.addCTTable(pointer.getGL98VALUE(),
							GlobalValidation.removeSubfWithNoRecord(raw[i]),
							Handler.convUpperCase(Handler.removeNumberSubf(raw[i])), tag[i], 1, 1, "T"));*/
					queryList.add(Indexing.addCTTable(pointer.getGL98VALUE(),
							raw[i],
							Handler.convUpperCase(raw[i]), tag[i], 1, 1, "T"));
				} else {
					queryList.add(Indexing.addCTTable(pointer.getGL98VALUE(),
							GlobalValidation.removeSubfWithNoRecord(raw[i]),
							Handler.convUpperCase(Handler.removeNumberSubf(raw[i])), tag[i], 1, 1, "T"));
...
if (next) {
	...
	if (isSuccess) {
		queryList.add(Indexing.deleteCTWORK(bufferno, Handler.tblName(module)));
		AuditTrail.InsertAudit("CT", "CTI0002", matno, modiby);
		out.println("Successful," + matno);
	} else {
		out.println("FAILINDEX");
	}
}

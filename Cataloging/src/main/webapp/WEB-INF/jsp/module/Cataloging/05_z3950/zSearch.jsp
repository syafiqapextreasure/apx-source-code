...
if (szRequestPage.equals("zList.jsp")) {
    ...
    int iFrom = 1;
    int iTo = 10;
    Vector<Object> vZClientThread = null;
System.out.println(szRequestPage);
    if (szRequestPage.equals("zList.jsp")) {
    	System.out.println("in" + (String) session.getAttribute("slcUseAttribute"));
        try {
    	...
    	System.out.println("ins21" + (String) session.getAttribute("slcUseAttribute"));
        try {
    ...
    vZClientThread = new Vector<Object>();
    ZClientConfigHandler handler = ZClientConfigHandler.getInstance();
    System.out.println("Size" + ArrayLengthCounter.countLength(aszServer));
    for (int i = 0; i < ArrayLengthCounter.countLength(aszServer); i++) {
    	
    	System.out.println("sss2" +handler.getZ3950Server(aszServer[i])+ iAttribute+ szSearchTerm+ iFrom+ iTo);
    ...
    System.out.println("er" + vZClientThread);
  ...
  /*   if (vZClientThread.size() > 0) {
    	response.sendRedirect("processingDelay");
    } */
%>

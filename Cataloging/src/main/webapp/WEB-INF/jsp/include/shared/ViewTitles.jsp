...
<%@ page import="com.ilmu.global.connection.DBConnection, 
					...
					com.ilmu.global.*,java.text.*,java.util.*,com.ilmu.cataloging.Global.ISBD"%>
	...
	<table class="table table-hover" id="view-result12" data-toggle="table" data-search="true" 
		...
		%>
			<tr  style="cursor:pointer" class="clickable-row" id="searchBtn1" data-action="<%=action%>" data-toggle="modal" data-matno="<%= i.getMatno() %>" data-dismiss='modal' data-type = <%=i.getType() %>>
				...
				<td class="title" data-value="<%= i.getTitle().replace("<","&#60").replace(">","&#62")  %>"><%= i.getTitle().replace("<","&#60").replace(">","&#62")  %></td>
				<td class="title" data-value="<%= Handler.getSubfield(i.getCallNo(), callisbd) %>"><%=Handler.getSubfield(i.getCallNo(), callisbd) %></td>

...
public class DeleteBO {
    ...
    public static boolean rcrdExist_CTWORK(String CT04MATNO) {
        String query = "SELECT CT04MATNO FROM CTWORK WHERE CT04MATNO='" + CT04MATNO + "'";
        boolean deletable = true;
        System.out.println(query);
        try {
            try {
    ...
    public static boolean rcrdExist_ACORDD(String AC03CTRLNO) {
        ...
        try {
            try {
                ...
                rs = stmt.executeQuery(query);
                while (rs.next()) {
                    deletable = Integer.parseInt(rs.getString("AC03CTRLNO")) <= 0;
                }
            }
            catch (SQLException e) {
                e.printStackTrace();
    ...
    public static boolean rcrdExist_SESERM(String SE01MATNO) {
        ...
        try {
            try {
                ...
                rs = stmt.executeQuery(query);
                while (rs.next()) {
                    deletable = Integer.parseInt(rs.getString("SE01MATNO")) <= 0;
                }
            }
            catch (SQLException e) {
                e.printStackTrace();
    ...
    public static void updateCTMATM(String CT02MATNO) {
        String query = "UPDATE CTMATM SET CT02STATUS='D', CT02DLTDATE='" + DateTime.displayToDBFormat(DateTime.getTodayDate()) + "', " + "CT02DLTIME='" + DateTime.timeToDBFormat(DateTime.getTodayTime()) + "'WHERE CT02MATNO = '" + CT02MATNO + "'";
        System.out.println(query);
        try {
            try {
                c = DBConnection.getConnection();
    ...
    public static boolean checkCTWORKD(String CT04MATNO) throws SQLException {
        boolean validate = false;
        c = DBConnection.getConnection();
        stmt = c.createStatement();
        try {
            String sql = "SELECT CT04STATUS FROM CTWORK WHERE CT04MATNO = '" + CT04MATNO + "'";
            System.out.println(sql);
            rs = stmt.executeQuery(sql);
            while (rs.next()) {
                if (!rs.getString("CT04STATUS").equals("D")) continue;
                validate = true;
            }
        }
        catch (Exception e) {

...
$(document).on("click", "#btn_submit", function(event){
	...
	var searchType =$("#patronsearch option:selected").data('value');
//	alert("1 " + searchType);
...
*/
   ...
   var table = $('#patr_result').DataTable({
        destroy: false,
        searching: false,
        bLengthChange: false,
        autoWidth: false,
...
$(document).ready(function() {
	...
	//$(document).on("click", "#btn_submit", function(event){
   $('#btn_submitIsy').off('click').on('click', function(event) {	
//	event.preventDefault();
	var criteria = $('#patronSearchInput').val();
	var searchType =$("#patronsearch option:selected").data('value');
	var cate_id = $('#cate_type').val();
	...
	var url = "SearchPatron?criteria=" + encodeURIComponent(criteria) +"&search_type=" +searchType + "&cate_id="+ cate_id+"&action="+ action + "&sort="+ sort;
	//alert("AHHHHHHHHHHHHHHHHHHHHh" + searchType);
	 $.ajax({
	        async: false,
	        url: url,
	        success: function(data) {
	        	$("#display_vendor").html(data);
	        }
...
function getRenewalItem(renewal){
	
	 var countRow = $('#table-charging tr').length;
	 
	var action = $( 'input[name=actiontype]:checked' ).val();
	
	var items = renewal.split(",");
	
	var overExpMem = "null";
	
	console.log(items[1], items[0]);

	var url = "Handler_PatronDetails?action="+action+"&CT03DOCNO=" + items[1];
	
	$.get(url,function(data)
	{
		if(data.trim()=="066,Recall"){
			messageBox("082","","");
		}else{
		$("#patrDet").html(data);
		  access(items[0]);
		  ...
		  {
			proceedRenewal(overExpMem);
		  }
		  
		  function trigOverideDdate(){
			...
			$.get(urls,function(flagExp){
				...
				if(ansFlagExp == '1'){
					...
					allowToProc = false;
					messageBox("068","","");
					console.log('Overide Expired Date Disable');
				}else{
					overExpMem = "OK";
					proceedRenewal(overExpMem);
				}
			...
			function proceedRenewal(overExpMem){
				$.get("AccessionRenewals?name=" + items[1] + "&patronid=" + items[0] + "&overExp=" + overExpMem,function(data)
		    			{ 
				  ...
				  if(data.trim()=="020"){
				...
				}else if(data.trim()=="107"){
					  messageBox("107","","@status");
				}else{
					
					$('#table-charging tr.disCT03DOCNO').remove();
					...
					$('#table-charging tr.status').remove();
						$(".hide").css('display', '');
						 ...
						 var time = currentDate.getHours()+":"+ currentDate.getMinutes()+":" + currentDate.getSeconds(); // => 51
						/* $('#table-charging tbody').prepend('<tr style="background-color:white" class="status"><td class="disCT03DOCNO">'+datas[0]+'</td>'+
						 ...
						 $("#receipting").prop("disabled",false);
						 $("#override").prop("disabled",false);
						 $('#table-charging tbody').prepend('<tr style="background-color:white" class="status">'+
								'<td class="rowNumber">'+1+'</td>'+ 
								'<td class="disCT03DOCNO">'+datas[0]+'</td>'+
								'<td class="Cdate">'+datas[2]+'</td>'+
								'<td class="Ctime">'+datas[3]+'</td><td class="Ddate">'+datas[4]+'</td>'+
								'<td>'+datas[5]+'</td><td class="statusid">'+datas[6]+'</td><td class="Rdate">'+datas[7]+
								'<input type="hidden" class="renewDate" value='+datas[7]+'></td><td class="Rtime">'+datas[8]+
								'<input type="hidden" class="renewTime" value='+datas[8]+'><td class="lateby">'+datas[9]+'</td><td class="fines">'+datas[10]+
								'</td></tr>');
						 
						 $.each($('#table-charging tbody .rowNumber'), function(i, v) {
						     $(this).text(i + 1);
						  });
						 swal.close();
						 /*
				  ...
				  if(data.trim()=="020"){
					...
					$('#table-charging tr.status').remove();
						$(".hide").css('display', '');
						 ...
						 var currTime = currentDate.toLocaleString('en-GB');
				//		 var time = currentDate.getHours()+":"+ currentDate.getMinutes()+":" + currentDate.getSeconds(); // => 51
						 var time = currTime.substring(11,20);
						 $(".title").html(datas[1]);
						 $('#table-charging tbody').prepend('<tr style="background-color:white" class="status">' +
								 '<td class="rowNumber">'+1+'</td>'+
								 '<td class="disCT03DOCNO">'+datas[0]+'</td>'+
...
function getResvDetails(CI03PATR){
	...
	$.get("Handler_AddResv",{CI03PATR:CI03PATR, CI03DOCNO:CI03DOCNO, CI03MATNO:CT03MATNO, username:username},function(result){
		var message = (result.trim()).split(",");
			...
			if(result.trim()=="ok"){
				setTimeout(function(){swal("Successful", "Reservation successfully added"); }, 1000);
					...
					$.get(url,function(data)
						...
						if ($('.ackLetter').is(":checked"))
 						{
 							//var url = "GeneratePreviewDocument?docno="+CI03DOCNO+"&matno=" + CT03MATNO+"&action=acknowledgement";
							//13112019
							var url = "GeneratePreview?docno="+CI03DOCNO+"&matno=" + CT03MATNO+"&action=acknowledgement";
				...
				messageBox("079","","");
			}else if(result.trim()=="101"){
				messageBox("101","","");
			}else if(result.trim()=="085"){
				messageBox("085","","");

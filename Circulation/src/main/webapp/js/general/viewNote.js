$(document).ready(function() {
	...
	if($(".GL14PATR").val() != ""){
		...
		idpatr = $("#hiddenPatrId").val();
	}
	var form = $("#formdataNote").val();
	console.log('inside js ' + idpatr);
	console.log('inside js form ' + form);
...
//										$('.swal2-confirm').click(
//											function() {
//												console.log('delete function swal 2 ??');
//												$('#table-note').DataTable().row(index).remove().draw();
//												//location.reload();
//										});
									}else{
	...
	/////close modal
	 $(".cancelNote, .closeModalNote, .closeNote").click(function () {
		...
		// $('#modal_vendorViewNoteContent').empty(); // Clear the modal content
		  console.log('Form has been reset.111');
     });
	 
	
	$('.modalViewNote').on('hidden.bs.modal', function () {
	    $('.modalViewNote').find('form').trigger('reset');
	    $('.formdataNote').data('bootstrapValidator').resetForm();
	    //$('#modal_vendorViewNoteContent').empty(); // Clear the modal content

...
<script type="text/javascript" src="<%= request.getContextPath() %>/plugin/datatables/jquery.dataTables.min.js"></script>
 ...
 <link rel="stylesheet" href="<%= request.getContextPath() %>/plugin/datatables/dataTables.bootstrap.css">
	...
	<table class="table table-hover" id="view-result12" data-toggle="table" data-search="true" 
		...
		%>
			<tr  style="cursor:pointer" class="clickable-row" id="searchBtn1" data-action="<%=action%>" data-docno="<%= i.getDOCNO()%>" data-toggle="modal" data-matno="<%= i.getMatno() %>" data-dismiss="modal">
			
				<td class="title" data-value="<%= i.getTitle() %>"><%= i.getTitle() %></td>
				<td class="callNo" data-value="<%= i.getCallNo() %>"><%= i.getCallNo() %></td>
				<td class="pubYear" data-value="<%= i.getYear() %>"><%= i.getYear() %></td>
				<td class="publ" data-value="<%= i.getPublication() %>"><%= i.getPublication() %></td>
				<td class="editon" data-value="<%= i.getEdition() %>"><%= i.getEdition() %></td>
				<td class="docno" data-value="<%= i.getDOCNO() %>"><%= i.getDOCNO() %></td>

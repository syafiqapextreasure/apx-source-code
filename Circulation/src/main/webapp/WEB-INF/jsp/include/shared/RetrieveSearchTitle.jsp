...
System.out.println(patrid+ criteria+ startDate+ stopDate + searchType);
		...
		if(action.equals("7") && buffer!=null){
			...
			{	
System.out.println("test" +searchType );
			...
			<%
					//}else{
			%>
				<tr  style="cursor:pointer" class="clickable-row" data-action="<%=action %>" data-bufferno="<%if((i.getRating())!=null){ %><%= i.getRating() %><%}else{%><%= criteria %><%}%>" data-type="<%=i.getPatrName()%>">
		...
		<% 			//}
			...
			if (count==0){
				...
				%>
				<tr  style="cursor:pointer" class="clickable-row" data-action="<%=action %>" data-matno="<%= criteria %>" data-dismiss="modal" data-docno="<%= i.getBufferno()%>" data-type="<%=i.getMatno()%>">

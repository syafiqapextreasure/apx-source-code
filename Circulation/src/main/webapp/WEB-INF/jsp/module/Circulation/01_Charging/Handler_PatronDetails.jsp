...
if(action.equals("smartcardreader")){
	 InetAddress myHost = InetAddress.getLocalHost();
	String patronid = SmartCardReader.getPatron_Id(request.getRemoteAddr());
	
	out.println(patronid);
}
else if(action.equals("discharge")||action.equals("renewal")){
	...
	if(((!(status1[0].trim()).equals("C")) && !(status1[0].trim()).equals("E"))&&action.equals("discharge")){
		out.println("066," + status1[1]);
	}else if(((!(status1[0].trim()).equals("C")))&&action.equals("renewal")){
		out.println("066," + status1[1]);
	}else{
%>
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
    <%@page import="com.ilmu.circulation.Charging.Patron,com.ilmu.circulation.Charging.Circulation,java.util.*,com.ilmu.global.*,
				java.util.*, java.text.*,com.ilmu.circulation.Charging.GeneralUtility"%>
							...
							<tr>
								<td colspan = "2">
							     ...
							     <!-- Removed temporarily -->
									...
									<button id="refresh" class="btn btn-primary"><span class="glyphicon glyphicon-refresh" style="color:white" title="Refresh"></span></button>
								</td>
							</tr>
							<tr>
								<td>	
							  ...
							  <tr>
							   <td>
								...
								<td>
									<label>Department:</label><span class="department"><%=i.getDeptDesc()%></span>
								</td>
							  </tr>
						</tbody>
				</table>
...
System.out.println(GL14PATR + "APANI");
if(GL14PATR=="")
{
	System.out.println("APANI 033");
	out.println("033");
}
else
{
	...
	System.out.println(errmessage + "APANI errmessage");
	
//Change the code to display both error message and patron details together - 12/6/2019
	...
	/* else
	{
		...
		if(errmessage.equals("036")){
			String responsestring = patr.getMsPatronName()+"/n"
					+patr.getMsPatronCateDesc()+"/n"
					+patr.getMsPatronStatusDesc(action)+"/n"
					+patr.getMsMemDate()+"/n"
					+patr.getMsMemExpiryDate()+"/n"
					+patr.getMsNoCircByPatron()+"/n"
					+patr.getNoMsGetItemOverdue()+"/n"
					...
					+patr.getMsGetTotalFinesOutstanding()+"/n"
					+patr.getMsGetTotalPaid() +"/n"
					+patr.getMsPatronBranchDesc() + "/n"
					+patr.getMsPatronDeptsDesc(GL14PATR) + "/n"
					+patr.getGL14MEMDATE() + "/n"
					+patr.getGL14EXPDATE()  + "/n"
					+ errmessage;
			out.println(responsestring);
		}else{

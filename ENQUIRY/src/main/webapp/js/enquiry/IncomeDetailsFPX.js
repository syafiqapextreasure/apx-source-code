 $(document).ready(function() {
	...
	$('#payData, #brnchData,#patronCate').multiselect({
    	allSelectedText: 'All',
   		maxHeight: 200,
	...
	$("#payData, #brnchData,#patronCate").multiselect("disable");
	...
	$("#chkBranch").on('change', function () {
	    ...
	    if (self.is(":checked")) {
			...
			$('#incomeReterive').prop('disabled', false);
			$("#brnchData").multiselect("clearSelection")
 			$("#brnchData").multiselect( 'refresh' ); 
	    }
	});
	...
	$('#incomeReterive').click(function(){
    	...
    	var brancodeValue = 'T3.GL14BRNC';
    	var url = "resultIncomeDetailFPX";
		var brancode = "patronB";
					...
					dom: 'Bfrtip',
						buttons: [
					             ...
					             },
								{
										...
										exportOptions: {
											columns: ':not(:eq(8))',  // Hides the 9th column (index starts at 0)
											search: 'applied',
											order: 'applied'
										},
										...
										customize: function (doc) {
											...
											doc.content[1].table.widths = [ '9%', '3%', '6%','7%', '9%', '11%', '8%', '11%','7%', '6%','6%', '8%','9%','8%'];
											//var cols = [];
   											

											//alert(rowCount +"rowCount1");
											for (i = 1; i < rowCount; i++) {
												doc.content[1].table.body[i][1].text = i
		...
		*/						                   text: [
						    ...
						    ajax: {
						        ...
						        dataSrc: function (json) {
						            ...
						            for(var i=0;i< json.length; i++){
						              ...
						              return_data.push({
										...
										'FPX Id' : json[i].trxnid,
										'FPX Reference' : json[i].trxnref,
										'Doc No' : json[i].docno,
						     	...
						     	columns    : [
									...
									{'data': 'Txn Date', className: "text-right"},
								...
								drawCallback: function ( settings ) {
									$('.branchtotal div').remove();
								            ...
								            total[group_assoc]=intVal(api.column(10).data()[i]);
								              ...
								              if ( last !== group ) {
								                    $(rows).eq( i ).before(
														'<tr class="group"><td colspan="8">'+group+'</td><td style="text-align:right" class="'+group_assoc+'"></td><td></td></tr>'
								                    );
								          ...
								          for(var key in total) {
											$("."+key).html( $.fn.dataTable.render.number(',','.','2','').display(total[key]));
	
											$('.branch_info').prepend("<div class='row branchtotal'><div class='num2'><div class='col-md-12 col-lg-12'><label><b>\n"+key
													+"</b></label><label><b>: </b></label><label><b> &nbsp"
													+"RM "+ $.fn.dataTable.render.number(',','.','2','').display(total[key])
													+"</b></label> </div></div> ");
							   ...
							   footerCallback: function ( row, data, start, end, display ) {
						            ...
						            // Update footer
									$("#allTotal").text(" RM "+$.fn.dataTable.render.number(',','.','2','').display(total));

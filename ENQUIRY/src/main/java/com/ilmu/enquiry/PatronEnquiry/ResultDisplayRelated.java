...
@WebServlet(value={"/ResultDisplayRelated"})
public class ResultDisplayRelated
extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String id = request.getParameter("id");
        List<Object> detailRelated = new ArrayList();
        detailRelated = PatronEnquiry.GetPatronDetails2(id);
        Gson gson = new Gson();
        JsonElement element = gson.toJsonTree(detailRelated, new TypeToken<List<PatronEnquiry>>(){}.getType());
        System.out.println("detailRelated : " + element);

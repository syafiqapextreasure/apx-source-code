		...
		$('#patronId').bind("enterKey",function(e){
	        ...
	        $.ajax({
	        	...
	        	success: function(data) {
	        		result = data;
	        		console.log(result);
	        		console.log(data[1].addr1);
	        		if(result != null) {
	 
	            		$('#patronIdinPS').val(value);
	    				$('#patronName').text(data[1].patronName);
	    				$('#patronAddr').append(data[1].addr1 +'\n'+ data[1].addr2+'\n'+ data[1].addr3+'\n'+ data[1].code+'\n'+ data[1].town);
	    				$('#membershipDate').text(data[1].memberDate);
	    				$('#expiredDate').text(data[1].expiredDate);
	    				$('#onloanCount').text(data[1].onloanItemCount);
						$('#overdueCount').text(data[1].onloanItemCount);
						$('#reservationCount').text(data[1].reservationCount);
						$('#itemChargedToDate').text(data[1].itemChargedToDate);
						$('#lateReturnToDate').text(data[1].lateReturnToDate);
						$('#noOfItemLost').text(data[1].noOfItemLost);
						$('#noOfSuspend').text(data[1].noOfSuspend);
						$('#itemChargedYTD').text(data[1].itemChargedYTD);
						$('#lateReturnsYTD').text(data[1].lateReturnsYTD);	
						$('#lastChargeDate').text(data[1].lastChargeDate);
						$('#lastReturnDate').text(data[0].lastReturnDate);
						$('#lastRenewDate').text(data[1].lastRenewDate);
						$('#amountOutstanding').text(data[1].amountOutstanding);
						$('#amountPaid').text(data[2].amountPaid);
						$('#groupId').text(data[1].groupId);
						$('#status').text(data[1].status);
						$('#DOB').text(data[1].DOB);
						$('#gender').text(data[1].gender);
						$('#dept').text(data[1].dept);
						$('#category').text(data[1].category);
	    	...
	    	if( (result !="") && (result[1].validateMembershipRenewal=="true")){
				
	            var return_data = new Array();
	           
	            $('#spanPatronName').text(result[1].patronId);
	            $('#spanPatronCate').text(result[1].patronName);
			...
			}else if(result !="" && result[1].validateMembershipRenewal=="false"){
		...
		$.ajax({
	        	...
	        	success: function(data) {
	        		if(data != null) {
	    				...
	    				$('#membershipDate').text(data[1].memberDate);
	    				$('#expiredDate').text(data[1].expiredDate);
	    	//			$('#recordedBy').text(data[1].recordedBy);
	    				$('#onloanCount').text(data[1].onloanItemCount);
						$('#overdueCount').text(data[1].onloanItemCount);
						$('#reservationCount').text(data[1].reservationCount);
						$('#itemChargedToDate').text(data[1].itemChargedToDate);
				...
				$.ajax({
					...
					data: { patronId: $("#patronId").val(), expDate: $("#expiredDate").text(), newExpDate: $("#newExpDate_modal").val(), recordedBy: liferayLogin, fee: $("#fee_modal").val()},
						success : function(data) {
							...
							if (data[0].newExpiryDate == "pass") {
								swal("Membership Renew Successfully ");
										
								$('#divNewExpDate').toggle();
								$('#newExpDate').html(result[0].category);	
								$('#membershipDate').html(result[0].fee);	
							}else{

...
//    	}
    ...
    var officetype2;
    $('input[type=radio][name=addressRadiosName]').change(function() {
		officetype2 = $(this).val();
		  //alert(officetype2);
    });
	...
	var officetype3 = $('input[name="addressRadiosName"]:checked').val();
            
            if(officetype3==undefined){
			 officetype3 = 'department';
			}

        if($.urlParam('value')=='on'){
	
			//alert('12312'+$("input[name='addressRadiosName']:checked").val());
           /* var table = $('#overdue_table').DataTable();
            ...
            var table = $('#overdue_table').DataTable();
		      ...
		      // Get the initially selected rows
		      var initialSelection = table.rows({ selected: true }).data().toArray();
		      //console.log("Initially Selected Rows:", initialSelection);
		
		      // Clear the selected rows
		      table.rows({ selected: true }).deselect();
		
		      // Get the updated selected rows
		      var updatedSelection = table.rows({ selected: true }).data().toArray();
		      //console.log("Updated Selected Rows:", updatedSelection);

            $.ajax({
                url: "GeneratePreviewDocument",
                dataType: 'json',
                data: {
                    json: JSON.stringify(initialSelection), 
            ...
            if(officetype2==undefined){
			 officetype2 = 'department';
			}

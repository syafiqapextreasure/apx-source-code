...
function getRenewalItem(discharge){
	var action = $( 'input[name=actiontype]:checked' ).val();
	...
	$.get(url,function(data)
	{
		$("#patrDet").html(data);
		  ...
		  else
		  {
			  ...
			  $.get("AccessionRenewals?name=" + items[1] + "&patronid=" + items[0],function(data)
		    			{ 
			...
			//			 $("#receipting").prop("disabled",false);
						 $("#override").prop("disabled",false);
						 $('#table-charging tbody').prepend('<tr style="background-color:white" class="status"><td class="no">'+1+'</td><td class="disCT03DOCNO">'+datas[0]+'</td>'+
...
function getDischargeItem(discharge){
	...
	$.get(url,function(data)
	{
		$("#patrDet").html(data);
		  ...
		  if(!items[1] )
			  {
			  consol.log("line 223 global.js !items[1]");
			  messageBox("020","","");
			  }
				  ...
				  if(data.trim()=="020"){
					...
					$('#table-charging tr.status').remove();
						$(".hide").css('display', '');
						 ...
						 $("#title").html(datas[1])
						 $('#table-charging tbody').prepend(
								 '<tr style="background-color:white" class="status"><td class="no">'+1+'</td><td class="disCT03DOCNO">'+datas[0]+'</td>'+
									'<td class="Cdate">'+datas[2]+'</td>'+
									'<td class="Ctime">'+datas[3]+'</td><td class="Ddate">'+datas[4]+'</td>'+
									'<td class="Dtime">'+datas[5]+'</td><td class="statusid">'+datas[6]+'</td>' + 
									'<td class="lateby">'+datas[7]+'</td><td class="fines">'+datas[8]+'</td><td class="Rdate">'+(day <= 9 ? '0' + day : day)  + "/" + (month <= 9 ? '0' + month : month) + "/" + year +'</td><td class="Rtime">'+time+'</td></tr>');
...
function getResvColPatrId(GL14PATR){
	
	$("#instructorCB").val(GL14PATR);
	var action = $('input[name=actiontype]:checked').val();
		var url = "Handler_PatronDetails?action=charge&GL14PATR=" + GL14PATR;
		...
		$.get(url,function(data)
			...
			var details = data.trim().split("/n");
				...
				$("#instructorTF").val(details[0]);
		});	
}

function getMemberDetail(GL14PATR){

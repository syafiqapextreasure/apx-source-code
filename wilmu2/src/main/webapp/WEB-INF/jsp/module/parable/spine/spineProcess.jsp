...
<%
	...
	if(action.equalsIgnoreCase("accession_no")){
		...
		System.out.println(unprintLbl);
		hashData = queryAcession.loadbyAcession(number1, number2, unprintLbl, "CT03SPINESTATUS");
		System.out.println("ds12" + hashData);
		...
		for(int j = 0; j < listData.size(); j++){
			...
			List<String> LLC = new ArrayList<String>();
 			/* if(book_spine.getCallNo().toString() != null || !book_spine.getCallNo().trim().toString().isEmpty()){
				String call_no = book_spine.getCallNo().trim();
				LLC =  generalUtil.LLCSplit(call_no);
		...
		System.out.println(new JSONObject(listData).toString());
	...
	if(action.equalsIgnoreCase("accession_no_range")){
		...
		QueryAcession queryAcession= new QueryAcession();
		hashData = queryAcession.loadbyAcessionRange(number1, unprintLbl, "CT03BLESTATUS");
		listData = (List<book_spine>)hashData.get("list");
		for(int j = 0; j < listData.size(); j++){
			book_spine book_spine = listData.get(j);
			System.out.println("Title" + book_spine.getTitle());
			if(book_spine.getCallNo().trim().toString() == null || book_spine.getCallNo().trim().toString() == ""){
				System.out.println("NULL");
			}
			GeneralUtil generalUtil = new GeneralUtil();
		...
		System.out.println(new JSONArray(listData).toString());
		out.println(new JSONArray(listData));
	}
	if(action.equalsIgnoreCase("call_no")){
		...
		hashData = queryCallNumber.loadByCallNumber(call_number1,call_number2, unprintLbl, "CT03SPINESTATUS");
		listData = (List<book_spine>)hashData.get("list");
		out.println(new JSONArray(listData));
	}
	...
	//	hashData = queryCatalogDate.loadByCatalogDate(catalog_date1, catalog_date2, unprintLbl, "CT03SPINESTATUS");
	...
	if(action.equalsIgnoreCase("control_no")){
		...
		hashData = queryControlNumber.loadByControlNumber(control_number1,control_number2, unprintLbl, "CT03SPINESTATUS");
		listData = (List<book_spine>)hashData.get("list");
		out.println(new JSONArray(listData));
	}
	...
	if(action.equalsIgnoreCase("smd")){
		...
		hashData = querySMD.loadBySMD(smd,catalog_date1,catalog_date2,index_date1,index_date2, unprintLbl, "CT03SPINESTATUS");
		listData = (List<book_spine>)hashData.get("list");
		out.println(new JSONArray(listData));
	}
	if(action.equalsIgnoreCase("item_cat")){
		...
		hashData = queryItemCategory.loadByItemCategory(item_category.trim(),catalog_date1,catalog_date2,index_date1,index_date2, unprintLbl, "CT03SPINESTATUS");
		listData = (List<book_spine>)hashData.get("list");
		out.println(new JSONArray(listData));
	}
	if(action.equalsIgnoreCase("branch")){
		...
		hashData = queryBranch.loadByBranch(branch.trim(),catalog_date1,catalog_date2,index_date1,index_date2, unprintLbl, "CT03SPINESTATUS");
		listData = (List<book_spine>)hashData.get("list");
		out.println(new JSONArray(listData));
	}
	if(action.equalsIgnoreCase("index_date")){
		...
		hashData = queryIndexDate.loadByIndexDate(index_date1, index_date2, unprintLbl, "CT03SPINESTATUS");
		listData = (List<book_spine>)hashData.get("list");
		out.println(new JSONArray(listData));
	}
	if(action.equalsIgnoreCase("title")){
		...
		hashData = queryTitle.loadByTitle(title, unprintLbl, "CT03SPINESTATUS");
		listData = (List<book_spine>)hashData.get("list");
		out.println(new JSONArray(listData));
	}

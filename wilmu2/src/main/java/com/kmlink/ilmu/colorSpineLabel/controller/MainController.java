...
//    }
    ...
    public void createLabel_ori(List<Records> listTable, ModelAndView model, String tableId, String accessionNo) {
        if (listTable == null || model == null || tableId == null || accessionNo == null) {
            throw new IllegalArgumentException("Parameters cannot be null");
        }

        for (Records record : listTable) {
            processRecord(record, model, tableId, accessionNo);
        }
        
        model.addObject("listTable" + tableId, listTable);
    }

    private void processRecord(Records record, ModelAndView model, String tableId, String accessionNo) {
        String classNo = record.getClassNo();
        if (classNo == null || classNo.isEmpty()) {
            return;
        }

        // Process location
        processLocation(record, model, tableId);

        // Process class number
        processClassNumber(record, model, tableId);

        // Process colors
        processColors(record, model, tableId);

        // Process item class
        processItemClass(record, model, tableId);

        // Add common attributes
        addCommonAttributes(model, tableId, accessionNo, record);
    }

    private void processLocation(Records record, ModelAndView model, String tableId) {
        String location = record.getLocation();
        String locationDisplay = record.getLocationDisplay();
        model.addObject("label" + tableId + "location", locationDisplay);

        List<Records> locationColors = recordsDao.getAllColour(location);
        for (Records locColor : locationColors) {
            String colorCode = locColor.getColourCode();
            model.addObject("label" + tableId + "colcode", formatrgb(colorCode));
        }
    }

    private void processClassNumber(Records record, ModelAndView model, String tableId) {
        String classNo = record.getClassNo();
        model.addObject("label" + tableId + "classno", classNo);

        String[] parts = classNo.split("\\s+");
        String authMark;
        String callNo;

        if (parts.length >= 3) {
            authMark = parts[0] + "." + parts[1];
            callNo = parts[2];
        ...
        model.addObject("label" + tableId + "AuthMark", authMark);
        model.addObject("label" + tableId + "CallNo", callNo);
        model.addObject("label" + tableId + "callno", record.getCallNo());
    }

    private void processColors(Records record, ModelAndView model, String tableId) {
        String classNo = record.getClassNo();
        
        // Process first color
        processColor(record, model, tableId, classNo, 0, 1, "colour1");
        
        // Process second color
        processColor(record, model, tableId, classNo, 1, 2, "colour2");
        
        // Process third color
        if (classNo.length() == 2) {
            model.addObject("label" + tableId + "colour3", formatrgb("white"));
        } else {
            processColor(record, model, tableId, classNo, 2, 3, "colour3");
        }
        
        // Process fourth color
        if (classNo.length() > 3) {
            processColor(record, model, tableId, classNo, 4, 5, "colour4", true);
        } else {
            model.addObject("label" + tableId + "colour4", formatrgb("white"));
        }
    }

    private void processColor(Records record, ModelAndView model, String tableId, 
            String classNo, int start, int end, String colorName) {
        processColor(record, model, tableId, classNo, start, end, colorName, false);
    }

    private void processColor(Records record, ModelAndView model, String tableId, 
            String classNo, int start, int end, String colorName, boolean isCutter) {
        List<Records> colors = isCutter ? 
            recordsDao.findCutterColour(classNo.substring(start, end)) :
            recordsDao.findColour(classNo.substring(start, end));
            
        for (Records color : colors) {
            model.addObject("label" + tableId + colorName, formatrgb(color.getColourCode2()));
        }
    }

    private void processItemClass(Records record, ModelAndView model, String tableId) {
        List<Records> itemClasses = recordsDao.findItemClass(record.getItemCategory());
        for (Records itemClass : itemClasses) {
            model.addObject("label" + tableId + "itemclass", itemClass.getItemClass());
            model.addObject("label" + tableId + "colour5", formatrgb(itemClass.getColourCode2()));
        }
    }

    private void addCommonAttributes(ModelAndView model, String tableId, String accessionNo, Records record) {
        String status = Character.isDigit(record.getClassNo().charAt(0)) ? "nonfiction" : "fiction";
        String barcode = generateCode39BarcodeImage(accessionNo);
        
        model.addObject("barcode" + tableId, barcode);
        model.addObject("status" + tableId, status);
        model.addObject("accessionNo" + tableId, accessionNo);
    }

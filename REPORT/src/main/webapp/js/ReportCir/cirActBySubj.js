$(document).ready(function() {
	...
	$("input:checkbox[value='brnchCheckbox']").on('change', function () {
	    var self = $(this);
	    if (self.is(":checked")) {
			$('#branch').multiselect("enable");
			$('#branch').change(function() {
	...
	$('#Reterive').click(function(){
		...
		var searchCateriavaltext;
		
		switch(chkBoxSearchCateria) {
		  case 'brnchCheckbox':
			searchCateriaval = $('#branch').val();
			searchCateriavaltext = " In " +$('#branch').val();
			titleHeader = "Branch";
		...
		var t = $('#reportTable').DataTable( {
            ...
            buttons: [
                /*'excel', 'pdf', 'print'*/
						{
					  		extend: 'excelHtml5',
					        filename: 'WILMU_CirActivityBySubject', //' '+sDate + " until " +eDate,
					        title: 'Circulation Activity By Subject ' +searchCateriavaltext +sDate + " until " +eDate, 
					 	...
					 	},
						{
							...
							extend: 'pdfHtml5',
							filename: 'WILMU_CirActivityBySubject',
							...
							//doc.content[1].table.body[0][3].alignment = 'right';
							 doc.content[1].table.widths = [ '8%', '20%', '27%', '15%', '15%', '15%']; 
							// doc.content[1].table.widths = [ '8%', '55%', '12%', '12%', '12%']; 
							 //doc.content[1].table.body[0][5].alignment = 'right';
											...
											doc.content.splice(0, 1, {
						                        text: [
						                                   { text: 'Circulation Activity By Subject ' +searchCateriavaltext +sDate + " until " +eDate+ '\n',bold:true,fontSize:13,alignment: 'center'},
						                                   /////{ text: $("#libname").val() +'\n',bold:true,fontSize:13,alignment: 'center'},
						                        ],
						                        margin: [0, 0, 0, 12],
						                    });
					...
					
               			{
		                	extend: 'csv',
		                	filename: 'WILMU_CirActivityBySubject',
		                	title: 'Circulation Activity By '+$('#branch').text(),
							customize: function(doc){
										...
										return 'Circulation Activity By Subject ' +searchCateriavaltext +sDate + " until " +eDate +"\n\n" + doc;
										//var split_csv = doc.split("\n");
		    ...
		    ajax: {
		        ...
		        dataSrc: function (json) {
		            ...
		            for(var i=0;i< json.length; i++){
		              ...
		              return_data.push({
		                'No': (i+1),
						'Class No Range' : json[i].Id,
		                'Description' : json[i].Column1,
		                'Charge' : json[i].Column2,
		                'Discharge' : json[i].Column3,
		                'Renewal' : json[i].Column4,
		            })
		            }

$(document).ready(function() {
	...
	$(".datelabel").html("<b>Renewal Date Range</b>");
		...
		var t = $('#reportTable').DataTable( {
            ...
            buttons: [
                /*'excel', 'pdf', 'print'*/
						{
					  		extend: 'excelHtml5',
					        filename: 'WILMU_ItemRenewal',
					        title: 'Item Renewal Report By  '+titleHeader + +sDate + " until " +eDate,
						  ...
						  doc.content[1].table.widths = [ '2%', '10%', '17%', '5%', '5%', '5%','5%','5%','5%','5%','5%','6%','5%','5%','5%','5%','5%'];  
							 //doc.content[1].table.body[0][5].alignment = 'right';
											...
											doc.content.splice(0, 1, {
						                        text: [
						                                   { text: 'Item Renewal Report By  '+titleHeader +sDate+ ' until '+eDate+ ' \n',bold:true,fontSize:13,alignment: 'center'},
						                                   /////{ text: $("#libname").val() +'\n',bold:true,fontSize:13,alignment: 'center'},
						                        ],
						                        margin: [0, 0, 0, 12],
						                    });
					...
					
               			{
							...
							customize: function(doc){
										...
										return "Item Renewal Report By  "+titleHeader+ " from " +sDate + " until "+ eDate +"\n\n" + doc;
										//var split_csv = doc.split("\n");
		        ...
		        dataSrc: function (json) {
					...
					if(title!=undefined){
		              ...
		              return_data.push({
						...
						'Branch' : json[i].Column6,
						'Category' : json[i].Column7,
						'SMD' : json[i].Column8,
						'Patron Category' : json[i].Column9,
						'Department' : json[i].Column10,
						'No Renewal' : json[i].Column11,
						'Status' : json[i].Column12,
						'Last Renewal Date' : json[i].Column13,
						'Borrow Date' : json[i].Column14,
						'Due Date' : json[i].Column15,
						'Return Date' : json[i].Column16,
		            })
		            }
		     	...
		     	columns    : [
					...
					{'data': 'Department'},
					{'data': 'No Renewal', className: "text-right"},
					{'data': 'Status'},
					{'data': 'Last Renewal Date'},
					{'data': 'Borrow Date'},
					{'data': 'Due Date'},
					{'data': 'Return Date'},

		...
		var t = $('#reportTable').DataTable( {
            ...
            buttons: [
                /*'excel', 'pdf', 'print'*/
						{
					  		extend: 'excelHtml5',
					        filename: 'WILMU_TitleListing',
					        title: 'Title Listing By '+titleHeader +" from " +sDate + " until " +eDate,
					 	...
					 	},
						{
						   ...
						   doc.content[1].table.widths = [ '3%', '10%', '20%', '6%', '6%', '6%','6%','6%','6%','6%','9%','8%','8%'];  
							 //doc.content[1].table.body[0][5].alignment = 'right';
											...
											doc.content.splice(0, 1, {
						                        text: [
						                                   { text: 'Title Listing By '+titleHeader+' from '+sDate+ ' until '+eDate+ ' \n',bold:true,fontSize:13,alignment: 'center'},
						                                   /////{ text: $("#libname").val() +'\n',bold:true,fontSize:13,alignment: 'center'},
						                        ],
						                        margin: [0, 0, 0, 12],
						                    });
					...
					
               			{
							...
							customize: function(doc){
										...
										return "Title Listing By "+titleHeader+ " from " +sDate + " until "+ eDate +"\n\n" + doc;
										//var split_csv = doc.split("\n");
		    ...
		    ajax: {
		        ...
		        data : {startSentDate : startSentDate, endSentDate : endSentDate,  brnch : JSON.stringify(brnch),
						...
						type : type, dateSelection : dateSelection
						},
		        type: "GET",
		        dataSrc: function (json) {
					...
					var edition = json[i].Column7;
					
					if(edition!=undefined){
						if(edition.charAt(0)=='*'){
							edition = edition.slice(1);
							edition = "<i>"+edition+"</i>";
						}	
					}else{
						edition = "";
		     	...
		     	columns    : [
					...
					{'data': 'Call No'},
					{'data': 'Accession No'},
					{'data': 'Local Price', className: "text-right", render: $.fn.dataTable.render.number(',', '.', 2, '')},
					{'data': 'Vendor'},
					{'data': 'Author'},
					{'data': 'Edition'},
					{'data': 'Publisher'},

	...
	alert("checkedValue"+checkedValue);*/
	 ...
	 if(checkedValue=="index"){
		 if(newSearch=="selection"){
			...
			else if(newSearch=="ctrlNo" || newSearch=="MatNo"){
				$("#search_title").collapse('hide');
				$("#result_title").collapse('show');
				loader("#display_title");
			...
			$.get("Search_OfficerID",{criteria:searchtype,search_type:newSearch,
									  ...
									  stopDateInput:stopDateInput,type:type,record:title},function(data_title){
			...
			var buffer = "bufferno";
		   if(newSearch=="bufferNo" || newSearch=="MatNo"){
				var title = $('#retrieveBO #criteria').val();
				...
				$.get("RetrieveSearchTitle",{criteria:title,search_type:newSearch, buffer:buffer, tag:tag, action:action,type:type,module:module},function(data_title){
					...
					$.get("Search_OfficerID",{criteria:searchtype,search_type:newSearch,
											  action:action, startDateInput: startDateInput,
											  stopDateInput:stopDateInput, buffer:"bufferNo",type:type,
											  record:title},function(data_title){
						$("#officerSearch").modal('show');
						$("#officerResult").html(data_title);
						// $(data_title).appendTo('.modal-content').modal();
					});
...
$('.clickable-row').click(function() {
		...
		$.get("ViewTitles",{pointer : pointer, action:action},function(data){
			...
			/*	$("#result").collapse('show');*/
				$('#titleSearch').modal('hide');
				if(searchtype=="matno"){
					$.get("Accession_Maint",{txtMatNo : $(this).attr("data-accno"),actionID:"Table_SearchByAccNo"},function(data){
						$(".accessionList").html(data);
					});
				}else{
				$.get("Accession_Maint",{controlNoInput : matno,actionID:"Table_CTResultMaster"},function(data){
					$(".accessionList").html(data);
				});
				}
			}
		...
		*/	
					url = "BO?actionID=Handler_BORecord&matno="+matno+"&action=Index&module="+module;
					$('.collapse').show();
					loader("#tags");
							$.get(url,function(data_title){
								//$('.modal').removeClass('show');
			...
			url = "BO?actionID=Handler_BORecord&matno="+bufferno+"&action=Buffer&module="+module;

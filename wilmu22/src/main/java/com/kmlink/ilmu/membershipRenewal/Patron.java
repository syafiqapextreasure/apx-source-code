...
import com.kmlink.ilmu.membershipRenewal.GeneralUtility;
...
public class Patron {
    ...
    public String GetLastRenewDate(String msPatronId) {
        String sql = "SELECT CI02RDATE FROM CICIRC WHERE CI02PATR = '" + msPatronId + "' AND  ROWNUM = 1 " + "ORDER BY  CI02RDATE DESC";
        System.out.println(String.valueOf(sql) + " GetLastRenewDate");
        String msLastRenewDate = null;
        try {
            ResultSet rsObj = stmt.executeQuery(sql);
            while (rsObj.next()) {
                msLastRenewDate = Handler.ifIsNull(GeneralUtility.StrToDate(rsObj.getString("CI02RDATE")));
            }
        }
        catch (SQLException e) {
            e.printStackTrace();
    ...
    public static List<Patron> getPatronDetails(String msPatronID) throws SQLException {
        ...
        try {
            try {
                ...
                while (rsObj.next()) {
                    Patron circ = new Patron(Handler.ifIntIsNull(rsObj.getString("GL14LOSTBOK")), Handler.ifIntIsNull(rsObj.getString("GL14SUSPEND")), Handler.ifIsNull(GeneralUtility.StrToDate(rsObj.getString("GL14LASTBOR"))), GeneralUtility.StrToDate(rsObj.getString("GL14LASTRET")));
                    PatronDetails.add(circ);
                }
            }
            catch (Exception e) {
    ...
    public static List<Patron> retrievePatron(String criteria, String searchType, String cate_id, String sort) throws SQLException {
        System.out.println("in retreivePatron function");
        ArrayList<Patron> patrMisc = new ArrayList<Patron>();
        try {
            try {
                conn = DBConnection.getConnection();
    ...
    public static List<Patron> retrieveDischarging(String criteria, String searchType, String cate_id) throws SQLException {
        ...
        try {
            try {
                ...
                System.out.println("sql retrieve discharging: " + sql);
                System.out.println(cate_id);
                if (!cate_id.equals("0")) {
                    sql = String.valueOf(sql) + " and GL14CATE='" + cate_id + "'";
                }

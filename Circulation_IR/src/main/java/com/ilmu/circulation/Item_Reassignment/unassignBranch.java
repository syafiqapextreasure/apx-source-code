...
@WebServlet(value={"/unassignBranch"})
public class unassignBranch
extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        System.out.println(String.valueOf(CT03LOCA) + "CI07SICAT");
        try {
            con = DBConnection.getConnection();
            String query = "UPDATE CTDOCM SET CT03LOCA = ? WHERE CT03DOCNO = ?";
            System.out.println(query);
            PreparedStatement statement = con.prepareStatement(query);
            statement.setString(1, CT03LOCA);
            statement.setString(2, CT03DOCNO);
            statement.executeUpdate();
            String sql = "DELETE FROM CITLOCA WHERE  CI13ACCESSION = ?";
            System.out.println(query);

...
public class Patron {
    ...
    public Patron(String CI02PATR, String GL14NAME, String GL07DESC, String GL14MEMDATE, String GL14EXPDATE, String GL71DESC, String GL08DESC, String onloan, int overdue, int reserve, int onhold, double outstanding, double charged) {
    ...
    public void viewtable(String msPatronID) {
        ...
        System.out.println("Patron Details " + sql);
        try {
            rsObj = stmt.executeQuery(sql);
            if (!rsObj.next()) {
                this.errmessage = "036";
            } else {
                System.out.println("Extracting Patron Details");
                this.setMsPatronId(rsObj.getString("GL14PATR"));
                System.out.println(rsObj.getString("GL14PATR"));
                this.setMsPatronName(rsObj.getString("GL14NAME"));
                this.setMsPatronTitle(rsObj.getString("GL14NAMETITLE"));
                this.setMsPatronStatus(rsObj.getString("GL14STAT"));
                this.setMsPatronOldIC(rsObj.getString("GL14NEWIC"));
                this.setMsPatronNewIC(rsObj.getString("GL14OLDIC"));
                this.setMsPatronCode(rsObj.getString("GL14CODE"));
                this.setMsPatronTown(rsObj.getString("GL14TOWN"));
                this.setMsPatronCode2(rsObj.getString("GL14CODE2"));
                this.setMsPatronTown2(rsObj.getString("GL14TOWN2"));
                this.setMsPatronHTel(rsObj.getString("GL14HTEL"));
                this.setMsPatronOTel(rsObj.getString("GL14OTEL"));
                this.setMsPatronFax(rsObj.getString("GL14FAX"));
                this.setMsPatronCategory(rsObj.getString("GL14CATE"));
                this.setMsPatronHomeAdd1(rsObj.getString("GL14ADD1"));
                this.setMsPatronHomeAdd2(rsObj.getString("GL14ADD2"));
                this.setMsPatronHomeAdd3(rsObj.getString("GL14ADD3"));
                this.setMsPatronOHomeAdd1(rsObj.getString("GL14ADD21"));
                this.setMsPatronOHomeAdd1(rsObj.getString("GL14ADD22"));
                this.setMsPatronOHomeAdd1(rsObj.getString("GL14ADD23"));
                this.setMsPatronDOB(rsObj.getString("GL14DOB"));
                this.setMsPatronSex(rsObj.getString("GL14SEX"));
                this.setMsPatronRace(rsObj.getString("GL14RACE"));
                this.setMsPatronPassword(rsObj.getString("GL14PASW"));
                this.setMsPatronBranch(rsObj.getString("GL14BRNC"));
                this.setMsParentInfo(rsObj.getString("GL14PARENTID"));
                this.setMsPatronCourse(rsObj.getString("GL14COURSE"));
                this.setMsPatronDep(rsObj.getString("GL14DEPT"));
                this.setMsPatronSerialNo(rsObj.getString("GL14SECURE"));
                this.setMsMemExpiryDate(rsObj.getString("GL14EXPDATE"));
                this.setMsPatronImage(rsObj.getBytes("GL14IMG"));
                this.setMsMemDate(GeneralUtility.StrToDate(rsObj.getString("GL14MEMDATE")));
                this.setMsMemExpiryDate(GeneralUtility.StrToDate(rsObj.getString("GL14EXPDATE")));
            }
        }
        catch (Exception e) {
            e.printStackTrace();
        ...
        if (validPatr) {
            ...
            if (date1.compareTo(date2 = dateFormat.parse(msPatronExpDate)) < 0) {
                ...
                if (ableCharge) {
                    ...
                    System.out.println("sd" + this.getNoMsGetItemOverdue() + patrFineLimit + this.getMsGetTotalFinesOutstanding());
                    if (this.getMsGetTotalFinesOutstanding() <= patrFineLimit || patrFineLimit == 0.0) {
                        if (this.patronCateAllowedOverdue()) {
                            bSucessful = true;
                        } else if (this.getNoMsGetItemOverdue() != 0) {
                            this.errmessage = "046";
                            bSucessful = false;
                        } else {
                            bSucessful = true;
    ...
    public static boolean checkValidPatr(String GL14PATR) {
        boolean deletable;
        block12: {
            String query = "SELECT COUNT(*) AS Count FROM GLPATR WHERE GL14PATR = '" + GL14PATR + "'";
            deletable = true;
            System.out.println(query);
            try {
                try {
    ...
    public static List<Patron> PatronDetails(String CT03DOCNO) {
        ...
        String sql = "Select CI02PATR, GL14NAME, GL07DESC, GL14MEMDATE, GL14EXPDATE, GL71DESC, GL08DESC FROM CICIRC, GLPATR, GLCATE, GLBRNC, GLSTAT WHERE GL14PATR=CI02PATR and GL07CATE=GL14CATE and GL14BRNC = GL71BRNC and GL08STAT=GL14STAT and CI02PATR = '" + patrid + "' and CI02DOCNO ='" + CT03DOCNO + "' and CI02FLAG = 'C'";
        ...
        try {
            ...
            while (rsObj.next()) {
                Patron patronDetail = new Patron(rsObj.getString("CI02PATR"), rsObj.getString("GL14NAME"), rsObj.getString("GL07DESC"), rsObj.getString("GL14MEMDATE"), rsObj.getString("GL14EXPDATE"), rsObj.getString("GL71DESC"), rsObj.getString("GL08DESC"), onloan, overdue, reserve, onhold, outstanding, charged);
    ...
    public Double getFineLimit(String GL14PATR) {
        ...
        double fineLimit = 0.0;
        try {
            rsObj = stmt.executeQuery(sql);
            while (rsObj.next()) {
                fineLimit = Double.parseDouble(rsObj.getString("GL07FINELIMIT"));
            }
        }
        catch (SQLException e) {
            e.printStackTrace();
    ...
    public String getMsMemDate(String msPatronID) {
        ...
        try {
            ResultSet rsObj = stmt.executeQuery(sql);
            while (rsObj.next()) {
                this.msMemDate = GeneralUtility.StrToDate(String.valueOf(rsObj.getString("GL14MEMDATE")));
            }
        }
        catch (SQLException e) {
            e.printStackTrace();
    ...
    public String getMsMemExpiryDate(String msPatronID) {
        String sql = "Select GL14EXPDATE,GL14PATR From GLPATR Where GL14PATR = '" + msPatronID + "'";
        try {
            ResultSet rsObj = stmt.executeQuery(sql);
            while (rsObj.next()) {
                this.msMemExpiryDate = GeneralUtility.StrToDate(String.valueOf(rsObj.getString("GL14EXPDATE")));
            }
        }
        catch (SQLException e) {
            e.printStackTrace();
    ...
    public int GetLateReturn() {
        ...
        System.out.println(String.valueOf(sql) + " LATERETURN");
        int msLateReturn = 0;
        try {
            ResultSet rsObj = stmt.executeQuery(sql);
            while (rsObj.next()) {
    ...
    public String GetLastRenewDate(String msPatronId) {
        ...
        if (dbtype.getDBType().equals("mssql")) {
            ...
            System.out.println("oracle here");
            sql = "SELECT CI02RDATE FROM CICIRC WHERE CI02PATR = '" + msPatronId + "' " + "ORDER BY  CI02RDATE DESC LIMIT 1";
        }
        System.out.println(String.valueOf(sql) + " GetLastRenewDate");
        String msLastRenewDate = null;
        try {
            ResultSet rsObj = stmt.executeQuery(sql);
            while (rsObj.next()) {
                msLastRenewDate = Handler.ifIsNull(DateFormatter.DBToDisplayFormat(rsObj.getString("CI02RDATE")));
            }
        }
        catch (SQLException e) {
            e.printStackTrace();
    ...
    public boolean patronCateAllowedOverdue() {
        boolean allowoverdue = false;
        String sql = "SELECT GL07ALLOWOVD FROM GLCATE WHERE GL07CATE = '" + this.msPatronCategory + "'";
        System.out.println("Patron Category" + this.msPatronCategory);
        try {
            rsObj = stmt.executeQuery(sql);
            while (rsObj.next()) {
    ...
    public static List<Patron> PatrDetails(String msPatronID) throws SQLException {
        ...
        try {
            ...
            rsObj = stmt.executeQuery(sql);
            while (rsObj.next()) {
                Patron circ = new Patron(rsObj.getString("GL02NAME"), rsObj.getString("GL08DESC"), Handler.ifIsNull(GeneralUtility.StrToDate(rsObj.getString("GL14DOB"))), rsObj.getString("GL11NAME"), rsObj.getString("GL07DESC"), rsObj.getString("GL14SEX"));
    ...
    public static List<Patron> getPatronDetails(String msPatronID) throws SQLException {
        ...
        try {
            ...
            rsObj = stmt.executeQuery(sql);
            System.out.println(String.valueOf(sql) + " date");
            while (rsObj.next()) {
                Patron circ = new Patron(Handler.ifIntIsNull(rsObj.getString("GL14LOSTBOK")), Handler.ifIntIsNull(rsObj.getString("GL14SUSPEND")), Handler.ifIsNull(GeneralUtility.StrToDate(rsObj.getString("GL14LASTBOR"))), Handler.ifIsNull(DateFormatter.DBToDisplayFormat(rsObj.getString("GL14LASTRET"))));
                PatronDetails.add(circ);
            }
        }
        catch (Exception e) {
    ...
    public static List<Patron> patrMisc(String msPatronID) throws SQLException {
        ...
        try {
            conn = DBConnection.getConnection();
            stmt = conn.createStatement();
            String sql = "Select GL66DESC, GL65VALUE from GLPATREX, GLATTRI where GL66ATTRI = GL65ATTRI and GL65PATR='" + msPatronID + "'";
            rsObj = stmt.executeQuery(sql);
            while (rsObj.next()) {
                Patron misc = new Patron(rsObj.getString("GL66DESC"), rsObj.getString("GL65VALUE"));
                patrMisc.add(misc);
            }
        }
        catch (Exception e) {
        ...
        try {
            try {
                ...
                if (searchType.equals("patronId")) {
                    sql = criteria.contains("%") ? String.valueOf(sql) + "where UPPER(GL14PATR)  LIKE UPPER('" + criteria + "')" : String.valueOf(sql) + "where UPPER(GL14PATR) LIKE UPPER('%" + criteria + "%')";
                } else if (searchType.equals("patronName")) {
                    sql = criteria.contains("%") ? String.valueOf(sql) + "where UPPER(GL14NAME) LIKE UPPER('" + criteria + "')" : String.valueOf(sql) + "where UPPER(GL14NAME) LIKE UPPER('%" + criteria + "%')";
                } else if (searchType.equals("patrnic")) {
                    sql = criteria.contains("%") ? String.valueOf(sql) + "where GL14NEWIC LIKE '" + criteria + "' " : String.valueOf(sql) + "where GL14NEWIC LIKE '%" + criteria + "%' ";
                } else if (searchType.equals("patonic")) {
                    sql = criteria.contains("%") ? String.valueOf(sql) + "where GL14OLDIC LIKE '" + criteria + "' " : String.valueOf(sql) + "where GL14OLDIC LIKE '%" + criteria + "%' ";
                }
                System.out.println("bbbbbbbbbbbbbbbbbbbbbbbbbbbbb" + cate_id);

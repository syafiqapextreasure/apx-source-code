 	...
 	$('#btn-update').click(function(){
 		...
 		if(CT03SMD!="0"&&CT03DOCNO.length>0&&CT03MATNO.length>0&&CT03LOCA!="0"&&
   		...
   		var url = "Handler_EditAccMaint?CT03DOCNO=" + CT03DOCNO +
   				...
   				"&CT03VOL=" + $('#CT03VOL').val() +
   				"&CT03RATE=" + $('#CT03RATE').val().trim() +
   				"&foreignCost=" + $('#foreignCost').val().trim() +
   				"&localCost=" + $('#localCost').val().trim() +
   				"&sCost=" + $('#sCost').val().trim() +
   				"&hCost=" + $('#hCost').val().trim() +
   				"&copyType=" + copyType ;
   		...
   		$.ajax({
   			...
   			success: function(result) {
  				...
  				if(updateRecord == "ok"){
  					 alertMessage("Successful", "success", "005",null, null);
...
$(document).ready(function() {
	...
	$("#editAccMaint").on("shown.bs.modal", function(){
		 $(this).removeData();
	});
	
	$("#viewAccMaint").on("shown.bs.modal", function(){
		 $(this).removeData();
	});
	...
	swal({   
		...
		closeOnCancel: true 
			 ...
			 var deleteUrl = "Handler_DeleteAccMaint?accNo="+accNo;
				swal( 'Deleted!', accNo+ ' template has been deleted.','success');  
			      $.ajax({
			 			// Title
			 			url: deleteUrl,
			 			success: function(result) {

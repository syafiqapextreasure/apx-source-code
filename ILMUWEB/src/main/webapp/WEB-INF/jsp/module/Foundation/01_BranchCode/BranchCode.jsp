...
<script>
    ...
    <div class="panel-heading">
        ...
        <button class="btn btn-primary pull-right" data-toggle='modal' data-target='#vendorModal' href='SearchBranch'><i class="glyphicon glyphicon-search" title="Find Patron"></i></button>
        <button type="button" class="btn btn-primary pull-right" id="add" data-toggle='modal' data-target="#addModal" href="Modal_AddBranch"> 
    ...
    <div class="panel-body" id="display_vendor">
     ...
     <table id="example1" class="table table-bordered table-striped">
                    ...
                    <%
						...
						List<Foundation> list = eb.getAllBranchCode();
				
						for (Foundation e : list) {
							String code = e.getGL71BRNC();
					...
					%>
						<tr>
							<td><%=e.getGL71BRNC()%></td>
							<td><%=e.getGL71DESC()%></td>
							<%	if(e.getGL71DISPLAY()== null){%>
							<td name=GL71DISPLAY> </td>
							<%}else{%>
							<td><%=e.getGL71DISPLAY()%></td>
							...
							<button class="btn btn-primary btn-xs" data-toggle='modal' data-target='#viewModal' href='Modal_ViewBranch?GL71BRNC=<%=newConvertCode%>&&CODE=<%=e.getGL71TOWN()%>'><span class="glyphicon glyphicon-eye-open" title="View Record" ></span></button>
							<button class="btn btn-info btn-xs" data-toggle='modal' data-target='#editModal' href='Modal_EditBranch?GL71BRNC=<%=newConvertCode%>&&CODE=<%=e.getGL71TOWN()%>'><span class="glyphicon glyphicon-pencil" title="Edit Record" ></span></button>
							<button class="btn btn-dull btn-xs" onclick="deleteBranch(this.id)" id="<%=e.getGL71BRNC()%>"><span class="glyphicon glyphicon-trash" title="Delete Record" ></span></button>
   							</td>

...
<script>
	...
	<!-- <section class="content-header">
       <h1>
          <i class="glyphicon glyphicon-list"></i>  Currency Code
       </h1>
    ...
    <div class="panel-heading">
        ...
        <button class="btn btn-primary pull-right" data-toggle='modal' data-target='#vendorModal' href='SearchCurrency'><i class="glyphicon glyphicon-search" title="Find Currency"></i></button>
     ...
     <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                    	<%
							...
							List<Foundation> list = eb.getAllCurrencyCode();
				
							for (Foundation e : list) {
								
								String code = e.getGL24FOREX();
								//System.out.println(code+"code");
								
								String newConvertCode = code.replace(" ", "+");
						%>
						<tr>
							<td><%=e.getGL24FOREX()%></td>
							<td><%=e.getGL24DESC()%></td>
							<td><%=e.getGL24PRATE()%></td>
							<td><%=e.getGL24BRATE()%></td>
							
							<td class=" last">
							<input type="hidden" value="A" name="fndcategories"/>
							
							<button class="btn btn-primary btn-xs" data-toggle='modal' data-target='#viewModal' href='Modal_ViewCurrency?GL24FOREX=<%=newConvertCode%>'><span class="glyphicon glyphicon-eye-open" title="View Record" ></span></button>
							<button class="btn btn-info btn-xs" data-toggle='modal' data-target='#editModal' href='Modal_EditCurrency?GL24FOREX=<%=newConvertCode%>'><span class="glyphicon glyphicon-pencil" title="Edit Record" ></span></button>
   							<button class="btn btn-dull btn-xs" onclick="deleteCurrency(this.id)" id="<%=e.getGL24FOREX()%>"><span class="glyphicon glyphicon-trash" title="Delete Record" ></span></button>
   							</td>
...
<!-- ADD MODAL -->  
		<div class="modal fade" id="addModal" role="dialog" data-keyboard="false" data-backdrop="static">
			<div class="modal-dialog" style="width:max-width:90%;overflow:auto">

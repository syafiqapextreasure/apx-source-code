...
$(document).on("click", "#btn_submit", function(event){
	...
	$.get("SearchPatron",{criteria:criteria,search_type:searchType,cate_id:cate_id, action:action, sort:sort, textid:textid},function(data_vendor){
...
function appendData(GL14PATR, textid){
	...
	if(textid.trim() == "libid"){
		alert("twtwt"+GL14PATR);
		$("#txtLibraryId").val(GL14PATR);
		$("#txtLibraryId").focus();
	}else if(textid.trim() == "reqid"){
		$("#txtPatron").val(GL14PATR);
		$("#txtPatron").focus();
	}else if(textid.trim() == "libid2"){
		$("#txtLibraryID").val(GL14PATR);
		$("#txtLibraryID").focus();
	}else{

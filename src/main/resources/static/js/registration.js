...
// Update the total value
    ...
    // Enhanced event listener for payment table checkboxes
    // Enhanced event listener for payment table checkboxes
    document.getElementById('tanggunganTableBody').addEventListener('change', async function(event) {
        ...
        const rowNama = row.querySelector('.idPenggunaCell')?.textContent?.trim();

        // Main user row has namaPengguna as the ID Pengguna value (not empty)
        const isParentRow = rowNama === namaPengguna;

        if (checkbox.checked) {
            if (isParentRow) {
                // Check for main user duplicate
                const isDuplicate = await checkDuplicateOnCheckbox("/eforms/checkDuplicateNoKP", parentNoKP, "#errorNoKP");
                if (isDuplicate) {
                    checkbox.checked = false;
                    return;
                }
            } else if (icDependent) {
                // Check for dependent duplicate
                const isDuplicate = await checkDuplicateOnCheckbox("/eforms/checkDuplicateNoKPDep", icDependent, "#errorNoKPDep");
                if (isDuplicate) {
                    checkbox.checked = false;
                    return;
                }

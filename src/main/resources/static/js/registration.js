     ...
     var updatedCat=catogery();
				...
				$('input[name="Adakah_ahli_keluarga_anda_merupakan"]').change(function() {
					...
					if ($('input[name="Adakah_ahli_keluarga_anda_merupakan"]:checked').val()) {
					       var isStaf=      $('input[name="Adakah_ahli_keluarga_anda_merupakan"]:checked').val()=='ya'?true:false;
			               var desability  =$('#ya4').val=== 'ya' ? true:false;
			           $('input[name="Adakah_ahli_keluarga_anda_merupakan"]:checked').val()=='ya'?	$("#lampiranId1").attr("required", "required"):$("#lampiranId1").removeAttr("required");
		  	updateUserDetails(desability,isStaf);
		/*    	  var updatedCat=catogery(); 
		  	  */
		  	  
		  	  
		  	  if ($('input[name="Adakah_ahli_keluarga_anda_merupakan"]:checked').length > 0 &&
   ($('input[name="Adakah_ahli_keluarga_anda_merupakan"]:checked').val() == 'ya' || 
    $('input[name="Adakah_ahli_keluarga_anda_merupakan"]:checked').val() == 'tidak')) {
			        	 var updatedCat=catogery();
		  	        console.log("Updated category is is ",updatedCat);
		  	             $("#updatedCat").val(updatedCat);
					    }else{
		 ...
		 function generateSelectedList() {
		        ...
		        rows.forEach(row => {
		            ...
		            if (checkBox && checkBox.checked) {
		                ...
		                const nokPTanggungan =row.querySelector('.icDependent')?.textContent?.trim() || "null"; //row.querySelector('.icDependent').textContent;
		                selectedList.push({ idPengguna, hubungan, statusOKU, harga,nokPTanggungan });
 ...
 */
 ...
 $("#portalId").change(function() { 
		    ...
		    if(isStaff){
				...
				//console.log("noKPTanggungan==null ",noKPTanggungan==null);
		    var ic =noKPTanggungan==null? $("#noKP").val().trim():noKPTanggungan;
           // console.log("ic number is ",ic);

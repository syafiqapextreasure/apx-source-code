...
<%@ page import="com.ilmu.global.connection.DBConnection, 
					com.ilmu.Authority.AuthorityMaint.*,
					java.util.List, 
...
System.out.println("Testing");
	...
	System.out.println("action" + action);
     ...
     List<Authority_Record> Leader = null;
	if(action.equals("Index")){
		controlNoInput = request.getParameter("matno");
		System.out.println("333" + controlNoInput);
		CT02LEADER = Authority_Record.getLeader(controlNoInput);
		info = Authority_Record.getIndexCTMATM(controlNoInput);
		...
		//modidate = info.getCT02IDXDATE();
		System.out.println("IndexRecord" + info.getCT02STATUS() + info.getCT02IDXTIME() + "IndexRecord1" + 
		info.getCT02IDXDATE() + "IndexRecord2" + info.getCT02IDXBY() + info.getCT02MATNO() + info.getCT02TYPE());
		matno = info.getCT02MATNO();
		ctype=Tag_Handler.getMsBibType(info.getCT02TYPE());
		status = "Index Record";
		BORecord = Authority_Record.getRetrievedData(controlNoInput);
		lblNumbering = "Control No. ";
		Leader = Authority_Record.getLeader(controlNoInput);
		typecode = info.getCT02TYPE();
		//manda =Authority_Record.getLeaderManda("000");
	}else if(action.equals("Buffer")){
		controlNoInput = request.getParameter("matno");
		BORecord = Authority_Record.getCTWORK(controlNoInput);
		info = Authority_Record.getBufferCTMATM(controlNoInput);
		createby = info.getCT02CREBY();
		createtime = info.getCT02CRTIME();
		createdate = info.getCT02CREDATE();
		System.out.println("RR" + info.getCT02IDXBY());
		modiby = info.getCT02IDXBY();
		moditime = info.getCT02IDXTIME();
		modidate = info.getCT02IDXDATE(); 
		matno = controlNoInput;
		status = "Buffer Record";
		//Authority_Record getType = Authority_Record.getIndexCTMATM(Handler.replaceWith0(request.getParameter("matno")));
		//If type does not exist, will add as cataloging by default
		if((info.getCT04TYPE())==null){
			ctype=Tag_Handler.getMsBibType("C");
			typecode = "C";
		}else{
			System.out.println("Type" + info.getCT04TYPE());
			ctype=Tag_Handler.getMsBibType(info.getCT04TYPE());
		...
		BORecord = Authority_Record.getBOList(tplID);
		DefaultTag = Template_Maintenance.getDefault();
		...
		status = "New Record";
		lblNumbering = "Buffer No. ";
		ctype=Tag_Handler.getMsBibType(request.getParameter("type"));
...
%>
	...
	<tbody>
		...
		<tr>
			<td>Modified By:<span class="Muser"><%=modiby  %></span><input type='hidden' id='newUser' value='<%=(String)session.getAttribute("username")%> '></td>
	...
	<table class='table table-condensed table-fixed Authority_Record' id='Authority_Record' style='border-collapse:collapse;color:black;width:100%'>
			...
			<th>
				<div class='btn-group pull-right'><button type='button' data-toggle='dropdown' class='btn btn-default dropdown-toggle'>Action <span class='caret'></span></button>
					<ul class='dropdown-menu menu'>
						...
						<li class=''><a id='cancelDupCpy' class='disableCancelDuplicate' style='cursor:pointer'>Cancel</a></li>
		...
		<tbody>
			...
			<%
				int inc = 0;
			if(action.equals("Index")){
				for(Authority_Record i: CT02LEADER)
				{
			...
			%>			
						...
						<td><input type='text' value='<%= i.getCT02LEADER() %>' class="form-control data" size='100' data-manda=<%=i.getGL17MANDA()%> data-autflag='<%=i.getGL17AUTFLAG()%>' disabled ></td>
						<td>
			...
			<tr class="<%=inc%>">
				...
				<td>
					...
					<%
						...
						String desc = Authority_Record.getLinkTagDesc();
					%>

$(document).ready(function() {
	...
	$(".recordselectionDiv, .printerStat, .invstatDiv").hide();
	...
	////b) checkbox multiselect ordertype
	$("#chkBoxOrderType").on('change', function () {
	    var self = $(this);
	    if (self.is(":checked")) {
			$('#seordertype').multiselect("enable");
			$('#Reterive').prop('disabled', true);
			$('#seordertype').change(function() {
				if($("#seordertype").val() != ""){
					$('#Reterive').prop('disabled', false);
				}else{
					$('#Reterive').prop('disabled', true);
			...
			$('#seordertype').multiselect("disable");
			$('#Reterive').prop('disabled', false);
			$("#seordertype").multiselect("clearSelection")
 			$("#seordertype").multiselect( 'refresh' ); 
	    }
	});
	...
	$('#Reterive').click(function(){
		...
		//3 ordermode
		var seordertype = $("#seordertype").val(); 
		if(seordertype != ""){
		}else{
			seordertype = 0;
		}
		...
		var t = $('#reportTable').DataTable( {
            ...
            buttons: [
						{
					  		extend: 'excelHtml5',
					        filename: 'WILMU_SerListing',
					        title: 'Serials Listing ' +sDate +eDate,
					 	...
					 	},
						{
							...
							customize: function (doc) {
								...
								doc.content[1].table.widths = [ '3%', '12%', '10%', '9%', '9%', '9%', '9%', '9%', '9%', '9%', '12%'];
							...
							/*doc.content.push( {
											...
											doc.content.splice(0, 1, {
						                        text: [
						                                   { text: 'Serials Listing '+sDate +eDate+ '\n',bold:true,fontSize:13,alignment: 'center'}						                        ],
						                        margin: [0, 0, 0, 12],
						                    });
					...
					
               			{
							...
							customize: function(doc){
										...
										return "Serials Listing "+sDate + eDate +"\n\n" + doc;
									 }
             			},
            ],
			destroy: true,
		    ...
		    ajax: {
		        ...
		        data : {currencyFormat :  currencyFormat, 
						...
						seordertype : JSON.stringify(seordertype),
		        ...
		        dataSrc: function (json) {
		            ...
		            for(var i=0;i< json.length; i++){
		              ...
		              return_data.push({
		                ...
		                'Commencement Subs' : json[i].Column2,
		                'Frequency' : json[i].Column3,
		                'Price' : json[i].Column4,
						'PO No' : json[i].Column5,
						'PO Date' : json[i].Column6,
						'Subs Date' : json[i].Column7,
						'Expiry Date' : json[i].Column8,
						'Order No' : json[i].Column9,
						'Supplier' :json[i].Column10,
		            })
		            }
		     	...
		     	columns    : [
					{'data': 'No'},
					{'data': 'Title'},
					{'data': 'Commencement Subs'},
					{'data': 'Frequency'},
					{'data': 'Price', className: "text-right", render: $.fn.dataTable.render.number(',', '.', currencyFormat, '')},
					{'data': 'PO No'},
					{'data': 'PO Date', type: 'date-uk', targets: 0,},
					{'data': 'Subs Date', type: 'date-uk', targets: 0,},
					{'data': 'Expiry Date', type: 'date-uk', targets: 0,},
					{'data': 'Order No'},
					{'data': 'Supplier'},
				],

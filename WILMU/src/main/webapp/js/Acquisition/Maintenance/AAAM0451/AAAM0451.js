...
/*	
	...
	$('#orderingList').DataTable().rows().nodes().to$().find(':has(:checkbox:checked)').each(function(){
		...
		///orderno.push(order);02012020
		//orderno.push( "'"+order+"'");
		////orderno.push(order); 14012020
		orderno.push(order);
        //alert(dataArr);
    });
	 	$.get("Acquisition?type=Maintenance&module=AAAM0451&name=Handler_ProcessOrdering",{action:action, referenceNo:referenceNo, orderno:orderno.toString(), ordermode:selectedValue},function(data){
	if(data.trim()=="exist"){
		...
		/*	if(!document.getElementById("orderLetter").checked){
				...
				alert(url);
				//14102019
				var url = "Acquisition?type=Maintenance&module=AAAM0451&name=GenerateOrderDoc&orderno=" + orderno.toString()
				+ "&referenceNo=" + referenceNo + "&ordermode=" + selectedValue
		...
		if(data.trim()=="exist"){
			...
			// alertMessage("", "", "116","", "");
			 ...
			 if(selectedValue!="44"&&selectedValue!="60" ){
				 $.get("FundDistribution",{module:"AAAM0450", vendor:vendor, ref:referenceNo,totalOrder:total.toFixed(2), desc:"Acq : Ordering"},function(data){
...
function generate(){
	...
	$.get("Global?type=Handler&name=GenerateNo",{functions:"SERPONO", action: "ReviewListNo"},function(data){
...
function previewOrder(){
	...
	var liferayLogin = $("#liferayLogin").val();
	 ...
	 var letterid = "EQ40";
		...
		//+ "&action=orderList";
	 	//02012020
		 var url = "Acquisition?type=Maintenance&module=AAAM0451&name=GenerateOrderDoc&letterId=" + letterid
...
/*		 var url = "GeneratePreviewDocument?orderno=" + orderno.toString()
			...
			+ "&vendor=" +vendor
			+ "&action=orderLetter";*/
		  var url = "Acquisition?type=Maintenance&module=AAAM0451&name=GenerateLetterAcq&orderno=" + orderno.toString()
		  + "&referenceNo=" + referenceNo + "&ordermode=" + selectedValue1
			+ "&liferayLogin=" +liferayLogin
			+ "&vendor=" +vendor
			+ "&action=orderLetter";
...
function retrieveRecord(){
	//alert("sddsss");
		...
		$.get("Acquisition?type=Maintenance&module=AAAM0451&name=Table_Ordering",{action:action, criteria:checked, vendor:vendor, value:value, value1:value1, ordermode:ordermode},function(data){
	...
	*/
	...
	//****************************************** AREA FOR MODAL *****************************************//
	 ...
	 $('#modalOdrMaint').on('show.bs.modal', function (event) {
		...
		switch(recipient){
			case state = 3:
				modal.find('.modal-title').text("View Details");
				//viewDisplay();
				...
				$('#status').DataTable( {
					...
					bLengthChange: false,
				    ajax: {
				    	url: "getOrderMaint?type=status&rowid="+rowid,
				        type: "GET",
				...
				$('#requestor').DataTable( {
					...
					bLengthChange: false,
				    ajax: {
				    	 url: "getOrderMaint?type=requestor&rowid="+rowid,
				        type: "GET",
				        dataSrc: function (json) {
				            var return_data = new Array();
				...
				$('#feedback').DataTable( {
					...
					bLengthChange: false,
				    ajax: {
				    	 url: "getOrderMaint?type=feedback&rowid="+rowid,
				        type: "GET",
				        dataSrc: function (json) {
				            var return_data = new Array();
				...
				$('#errorInSupply').DataTable( {
					...
					bLengthChange: false,
				    ajax: {
				    	 url: "getOrderMaint?type=errorinsupply&rowid="+rowid,
				        type: "GET",
				        dataSrc: function (json) {
				            var return_data = new Array();
	...
	*/
	...
	$("#processOrder").click(function () {
		...
		if(selectedValue=="0"){
			...
			$.get("Acquisition?type=Maintenance&module=AAAM0451&name=Modal_ProcessV1",{mode:selectedText},function(data){
	...
	function alertMessage(code, criteria, label){

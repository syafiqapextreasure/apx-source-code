	...
	$('#Reterive').click(function(){
		...
		var t = $('#reportTable').DataTable( {
            ...
            buttons: [
						{
					  		extend: 'excelHtml5',
					        filename: 'WILMU_AcqInvoice',
					        title: 'Acquisition Status Report ' +sDate +eDate,
					 	...
					 	},
						{
							...
							doc.content.push( {
											...
											doc.content.splice(0, 1, {
						                        text: [
						                                   { text: 'Acquisition Status Report '+sDate +eDate+ '\n',bold:true,fontSize:13,alignment: 'center'}						                        ],
						                        margin: [0, 0, 0, 12],
						                    });
					...
					
               			{
							...
							customize: function(doc){
										...
										return "Acquisition Status Report "+sDate + eDate +"\n\n" + doc;
									 }
             			},
            ],
			destroy: true,
		        ...
		        dataSrc: function (json) {
		            ...
		            for(var i=0;i< json.length; i++){
		              ...
		              return_data.push({
		                ...
		                'Date Received' : json[i].Column3,
						'Order No' : json[i].Column4,
						'Price' : json[i].Column5,
						'Vendor' : json[i].Column6,
						'Branch' : json[i].Column7,
						'Title' : json[i].Column8,
						'Voucher No' : json[i].Column9,
						'Cheque No' : json[i].Column10,
						'Received' : json[i].Column11,
		            })
		            }
					...
					if(brnch != 0){
						...
						api.column(6, {page:'current'} ).data().each( function ( group, i ) {
							...
							var rowData = api.row(i).data();
			                    ...
			                    $(rows).eq( i ).before(
			                        '<tr class="group"><td colspan="11"><b> Branch: ' + group +'</b></td></tr>'
									+'<tr class="group"><td colspan="11"><b> Vendor : ' + rowData['Vendor'] +'</b></td></tr>'

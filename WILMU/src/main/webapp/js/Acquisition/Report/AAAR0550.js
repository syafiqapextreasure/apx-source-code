$(document).ready(function() {
	...
	$(".statusDiv, .acctDiv, .itemstatDiv").hide();
	...
	$('#Reterive').click(function(){
		...
		var t = $('#reportTable').DataTable( {
            ...
            buttons: [
						{
					  		extend: 'excelHtml5',
					        filename: 'WILMU_AcqReceivedItemsReport',
					        title: 'Received Items Report By ' +sDate +eDate,
					 	...
					 	},
						{
							 	...
							 	doc.content[1].table.widths = [ '3%', '9%', '9%', '8%', '13%', '9%', '7%', '8%', '3%', '7%', '7%', '9%', '8%']; 
							 		...
							 		doc.content.push( {
											...
											doc.content.splice(0, 1, {
						                        text: [
						                                   { text: 'Received Items Report By '+sDate +eDate+ '\n',bold:true,fontSize:13,alignment: 'center'}						                        ],
						                        margin: [0, 0, 0, 12],
						                    });
					...
					
               			{
							...
							customize: function(doc){
										...
										return "Received Items Report By "+sDate + eDate +"\n\n" + doc;
									 }
             			},
            ],
			destroy: true,
			...
			footer: true,
		    ajax: {
		    	url: "ResultAcqReceivedItemsReport",
		        ...
		        dataSrc: function (json) {
		            ...
		            for(var i=0;i< json.length; i++){
		              ...
		              return_data.push({
		                'No': (i+1),
		                'Received Date' : json[i].Column1,
		                'Accession' : json[i].Column2,
		                'ISBN' : json[i].Column3,
		                'Title' : json[i].Column4,
						'Vendor' : json[i].Column5,
						'Invoice No' : json[i].Column6,
						'Invoice Date' : json[i].Column7,
						'Currency' : json[i].Column8,
						'Foreign' : json[i].Column9,
						'Local' : json[i].Column10,
						'Order No' : json[i].Column11,
						'Order Date' : json[i].Column12,
		            })
		            }
		     	...
		     	columns    : [
					...
					{'data': 'Currency'},
					{'data': 'Foreign', className: "text-right", render: $.fn.dataTable.render.number(',', '.', currencyFormat, '')},
					{'data': 'Local', className: "text-right", render: $.fn.dataTable.render.number(',', '.', currencyFormat, '')},	
					{'data': 'Order No'},

$(document).ready(function() {
	...
	$("#chkBoxRangeInvoice").on('change', function () {
	    ...
	    if (self.is(":checked")) {
			...
			$("#startInput2, #endInput2").css("border", "");
	    }
	});
	...
	$("#startInput2, #endInput2").keyup(function(e){
		var startInput2 = $("#startInput2").val();
		var endInput2 = $("#endInput2").val();
		if(startInput2.length >=1 || endInput2.length >=1){
			$('#Reterive').prop('disabled', false);
		}else{
			$('#Reterive').prop('disabled', true);
		}
	...
	/////validate range number   #startInput2, #endInput2
	$("#startInput2, #endInput2").focusout(function(e){
		
		e.preventDefault();
		e.stopImmediatePropagation();


		var startnum = $("#startInput2").val();
		startnum = startnum.replace(/[^0-9]/gi, '');
		var startnumber = parseInt(startnum, 10); 
		
		
		var endnum = $("#endInput2").val();
		endnum = endnum.replace(/[^0-9]/gi, '');
		var endnumber = parseInt(endnum, 10); 

	    if(startnumber > endnumber){
			$("#startInput2, #endInput2").css("border", "solid red");
	        $("#Reterive").prop('disabled',true);
	
			messageBox("172","","");
			$("#chkBoxRangeInvoice").focus();
	    } else {
	        $(".error").css("display","none");
			$("#startInput2, #endInput2").css("border", "");
	        $("#Reterive").prop('disabled',false);        
	...
	$('#Reterive').click(function(){
		...
		var t = $('#reportTable').DataTable( {
            ...
            buttons: [
						{
					  		extend: 'excelHtml5',
					        filename: 'WILMU_AcqInvoiceReport',
					        title: 'Invoice Report ' +sDate +eDate,
					 	...
					 	},
						{
							...
							customize: function (doc) {
								...
								doc.content.push( {
											...
											doc.content.splice(0, 1, {
						                        text: [
						                                   { text: 'Invoice Report '+sDate +eDate+ '\n',bold:true,fontSize:13,alignment: 'center'}						                        ],
						                        margin: [0, 0, 0, 12],
						                    });
					...
					
               			{
							...
							customize: function(doc){
										...
										return "Invoice Report "+sDate + eDate +"\n\n" + doc;
									 }
             			},
            ],
			destroy: true,
		        ...
		        dataSrc: function (json) {
		            ...
		            for(var i=0;i< json.length; i++){
		              ...
		              return_data.push({
		                ...
		                'Order Mode' : json[i].Column4,
						'Reference No' : json[i].Column5,
						'Vendor' : json[i].Column7,
						'Payment Status' : json[i].Column8,
						'Amount' : json[i].Column9,
						'Date Created' : json[i].Column6,
		            })
		            }

$(document).ready(function() {
	...
	$('input[type=radio][name=reporttype]').change(function() {
		...
		$("#chkBoxRangeCallno").prop("checked", false);
		$("#startInputCallno, #endInputCallno").prop( "disabled", true);
		$('#Reterive').prop('disabled', false);
		$("#startInputCallno, #endInputCallno").val("");
		$("#startInputCallno, #endInputCallno").css("border", "");
		
		if (this.value == 'Unclassified') {
			
			//$('#catreportTable').DataTable().column(3).visible(true);
			$(".divTableUnclassified").show();
			$(".divTableUndefined").hide();
			$(".rangeCallnoDiv").show();
			
		} else if (this.value == 'Undefined') {
			
			$(".divTableUnclassified").hide();
			$(".divTableUndefined").show();
			$(".rangeCallnoDiv").hide();
		}
	...
	$('#Reterive').click(function(){
		...
		if (reporttype == 'Unclassified') {
			
			/////result display
		var t1 = $('#catreportTable').DataTable( {
			dom: 'Bfrtip',
            buttons: [
						{
					  		extend: 'excelHtml5',
			...
			footer: true,
		    ajax: {
		    	url: "ResultCatHoldingsReportUndefinedNUnclass",
		        data : {dateSelection : dateSelection,
						...
						startInputCallno : startInputCallno,
						endInputCallno : endInputCallno,
			...
			/////result display
		var t2 = $('#catreportTableUndefined').DataTable( {
			dom: 'Bfrtip',
            buttons: [
						{
					  		extend: 'excelHtml5',

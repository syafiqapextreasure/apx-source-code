	...
	$('#Reterive').click(function(){
		...
		var t = $('#listingreportTable').DataTable({
			...
			buttons: [
						{
					  		extend: 'excelHtml5',
					        filename: 'WILMU_Booking List',
					        title: 'WILMU Booking List ' + sDate +" until "+ eDate,
					 	...
					 	},
						{
						           ...
						           for (i = 0; i < rowCount; i++) {
										...
										doc.content[1].table.body[i][9].alignment = 'left';
											...
											doc.content.splice(0, 1, {
						                        text: [
						                                   { text: 'WILMU Booking List from '+sDate + " until " +eDate+ '\n',bold:true,fontSize:13,alignment: 'center'}						                        ],
						                        margin: [0, 0, 0, 12],
						                    });
					...
					
               			{
							...
							customize: function(doc){
										...
										return "WILMU Booking List from "+sDate + " until " + eDate +"\n\n" + doc;
									 }
             			},
            ],
			destroy: true,
		        ...
		        dataSrc: function (json) {
					...
					$.each( json, function( key, value ) {
						return_data.push({
							...
							'Room Title' : value.bookingRoomTitle,
							'Room Description' : value.bookingRoomDescr,
							'Date Booking' : value.dateBooking,
							'Start Date' : value.startDate,
			                'End Date' : value.endDate,
			                'Booked By' : value.personBookedName,

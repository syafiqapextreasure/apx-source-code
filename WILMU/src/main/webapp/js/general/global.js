...
/*$(document).on("click", "#btn_submit", function(event){
	...
	$.get("SearchPatron",{criteria:criteria,search_type:searchType,cate_id:cate_id, action:action, sort:sort},function(data_vendor){
		...
		$("#display_patronSearch").html(data_vendor);
	   });
});
*/
...
function getDetails(details) {
	...
	if (checkedValue == "index") {
		if (newSearch == "selection") {
			...
			$.get("Search_OfficerID", {
				criteria: searchtype, search_type: newSearch,
				action: action, startDateInput: startDateInput,
				stopDateInput: stopDateInput
			}, function(data_title) {
		...
		if (newSearch == "bufferNo") {
			...
			$.get("Search_OfficerID", {
				criteria: searchtype, search_type: newSearch,
				action: action, startDateInput: startDateInput,
				stopDateInput: stopDateInput, buffer: "bufferNo"
			}, function(data_title) {
				$("#officerSearch").modal('show');
				$("#officerResult").html(data_title);
				// $(data_title).appendTo('.modal-content').modal();
...
$('.clickable-row').click(function() {
	...
	if (pointer != null) {
		...
		$.get("RetrieveSearchTitle", {
			search_type: searchtype, patrid: patrid, startDate: startDate
			, action: action, endDate: endDate, criteria: criteria, buffer: "bufferNo"
		}, function(data_title) {
			$("#search_title").collapse('hide');
			$("#result_title").collapse('show');
			$("#display_title").html(data_title);
		...
		$.get("RetrieveSearchTitle", {
			search_type: searchtype, patrid: patrid, startDate: startDate
			, action: action, endDate: endDate, criteria: criteria
		}, function(data_title) {
			$("#search_title").collapse('hide');
			$("#result_title").collapse('show');
			$("#display_title").html(data_title);
